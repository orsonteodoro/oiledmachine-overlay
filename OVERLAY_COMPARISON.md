...
| Toolchain stability                 | Rolling compiler versions used by unstable distros        | LTS compilers versions used by LTS distro releases are       |
|                                     | releases or rolling only distros are preferred.  Possible | preferred.  Virtually no build time failures.                |
|                                     | build failure if the default C/C++ changes or possibily   |                                                              |
|                                     | new security issue (e.g. DoS, crash, or worst RCE).       |                                                              |
