...
| Containerization policy             | Decontainerize packages to be consistent with the build everything by source distro core value (lowers security and increases blast radius, contributes to or (subversively) promotes bad security architecture, is a bad habit or ignorance by an obsolete security team) | Containerize if EOL or if complex build, otherwise decontainerize.  Change possible to increase container ebuilds and use them as dependencies to better security architecture if community approves. |
