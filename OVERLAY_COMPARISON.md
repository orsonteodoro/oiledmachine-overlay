...
| AI ban                              | Y (opportunity cost, lowered security, more evil and less utilitarian, the [1] policy is more evil to the developer and unaffected by big tech which will continue doing what they will do, hypocritical use of energy by being a source based distro, and wasting more energy on non AI assisted rebuilds) | N (AI is used to fix trivial security compiler warnings like UAF and string format vulnerabilies that are typically rated critical to high severity; generate and secure ebuilds and artifacts (e.g. build files, config files, patches) with human critical oversight and change) |
