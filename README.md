...
for x in $(grep -l -e "^DESCRIPTION" $(find . -name "*ebuild")) ; do d=$(cat "${x}" | tr '\\' ' ' | grep "^DESCRIPTION" | cut -f 2 -d '"'); atom=$(echo "${x}" | cut -f 2-3 -d "/"); echo "| ${atom} | ${d} |" ; done | sort | uniq
-->
...
| app-arch/go-appimage | Purely experimental playground for Go implementation of AppImage   |
...
| app-benchmarks/mixbench | A GPU benchmark tool for evaluating GPUs and CPUs on mixed   |
| app-benchmarks/opencl-benchmark | A small OpenCL benchmark program to measure peak GPU/CPU performance. |
| app-benchmarks/transferbench | TransferBench is a utility capable of benchmarking simultaneous   |
...
| app-misc/alpaca | An Ollama client made with GTK4 and Adwaita |
| app-misc/amica | Amica is an open source interface for interactive communication with 3D characters with voice synthesis and speech recognition. |
...
| app-shells/ohmyzsh | A delightful community-driven framework for managing your zsh   |
| app-shells/percol | Adds flavor of interactive filtering to the traditional pipe   |
...
| dev-cpp/pystring | C++ functions matching the interface and behavior of python string   |
...
| dev-dotnet/aforgedotnet | AForge.NET Framework is a C# framework designed for developers and   |
| dev-dotnet/dotdevelop | DotDevelop will hopefully be a full-featured integrated development   |
...
| dev-dotnet/gtk-sharp | Gtk# is a Mono/.NET binding to the cross platform Gtk+ GUI toolkit   |
| dev-dotnet/libfreenect | Drivers and libraries for the Xbox Kinect device on Windows,   |
...
| dev-dotnet/monogame-extended | MonoGame.Extended are classes and extensions to make MonoGame more   |
| dev-dotnet/sdl2-cs | SDL2# - C# Wrapper for SDL2 |
| dev-dotnet/sfmldotnet | SFML.Net is a C# language binding for SFML |
| dev-dotnet/sharpnav | SharpNav is advanced pathfinding for C# |
| dev-dotnet/theorafile | Theorafile - Ogg Theora Video Decoder Library |
| dev-dotnet/tiledsharp | C# library for parsing and importing TMX and TSX files generated   |
| dev-dotnet/velcrophysics | High performance 2D collision detection system with realistic   |
| dev-games/enigma | ENIGMA, the Extensible Non-Interpreted Game Maker Augmentation,   |
| dev-games/gdevelop | GDevelop is an open-source, cross-platform game engine designed   |
...
| dev-lang/classic-flang | Flang is a Fortran language front-end designed for integration   |
| dev-lang/gambas | Gambas is a free development environment and a full powerful   |
...
| dev-lang/lua | A powerful light-weight programming language designed for   |
...
| dev-lang/typescript | TypeScript is a superset of JavaScript that compiles to clean   |
...
| dev-libs/atmi | ATMI is a runtime framework for efficient task management in   |
...
| dev-libs/hardened_malloc | Hardened allocator designed for modern systems. |
...
| dev-libs/nxjson | Very small JSON parser written in C. |
| dev-libs/protobuf | An extensible mechanism for serializing structured data |
| dev-libs/pugixml | DESCRIPTION=  |
...
| dev-libs/rccl-rdma-sharp-plugins | rccl-rdma-sharp plugin enables RDMA and Switch based collectives   |
...
| dev-python/JaroWinkler | Python library for fast approximate string matching using Jaro and   |
| dev-python/Levenshtein | The Levenshtein Python C extension module contains functions for   |
...
| dev-python/ale-py | The Arcade Learning Environment (ALE) -- a platform for AI research. |
...
| dev-python/autorom | A tool to automate installing Atari ROMs for the Arcade Learning   |
...
| dev-python/bsuite | bsuite is a collection of carefully-designed experiments that   |
...
| dev-python/dm-control | DeepMind's software stack for physics-based simulation and   |
...
| dev-python/dopamine-rl | Dopamine is a research framework for fast prototyping of   |
...
| dev-python/exhale | Automatic C++ library api documentation generation: breathe   |
...
| dev-python/flake8-import-order | Flake8 plugin that checks import order against various Python   |
...
| dev-python/flax | Flax is a neural network library for JAX that is designed for   |
...
| dev-python/gym | A toolkit for developing and comparing reinforcement learning   |
| dev-python/gym-notices | Gym Notices |
| dev-python/gymnasium | A standard API for single-agent reinforcement learning   |
...
| dev-python/keras-applications | Keras deep learning library reference implementations of deep   |
...
| dev-python/lightning | The Deep Learning framework to train, deploy, and ship AI   |
| dev-python/lightning-api-access | Lightning Frontend Showing how a given API can be accessed |
| dev-python/lightning-app | Use Lightning Apps to build everything from production-ready,   |
| dev-python/lightning-cloud | Lightning Cloud |
| dev-python/lightning-fabric | Fabric is the fast and lightweight way to scale PyTorch models   |
...
| dev-python/model-index | Create a source of truth for ML model results and browse it on   |
| dev-python/moviepy | Video editing with Python |
| dev-python/mujoco | Multi-Joint dynamics with Contact. A general purpose physics   |
| dev-python/multi-agent-ale-py | The Arcade Learning Environment (ALE) -- a platform for AI research. |
...
| dev-python/neural-compressor | SOTA low-bit LLM quantization (INT8/FP8/INT4/FP4/NF4) & sparsity;   |
...
| dev-python/open-spiel | OpenSpiel is a collection of environments and algorithms for   |
...
| dev-python/pettingzoo | A standard API for multi-agent reinforcement learning   |
...
| dev-python/python-chess | A chess library for Python, with move generation and validation,   |
...
| dev-python/pytorch-lightning | PyTorch Lightning is the lightweight PyTorch wrapper for ML   |
...
| dev-python/shimmy | An API conversion tool for popular external reinforcement   |
...
| dev-python/trfl | trfl is a library of building blocks for reinforcement learning   |
...
| dev-util/bear | Bear is a tool that generates a compilation database for clang   |
...
| dev-util/closure-compiler-npm | Check, compile, optimize and compress Javascript with   |
...
| dev-util/rocm-validation-suite | The ROCm Validation Suite is a system administrator’s and cluster   |
...
| media-gfx/material-maker | A procedural textures authoring and 3D model painting tool based   |
| media-gfx/nanovdb | A lightweight GPU friendly version of VDB initially targeting   |
| media-gfx/openvdb | Library for the efficient manipulation of volumetric data |
| media-gfx/upscayl | Upscayl - Free and Open Source AI Image Upscaler for Linux, MacOS and Windows built with Linux-First philosophy. |
...
| media-libs/libspng | libspng is a C library for reading and writing Portable Network   |
| media-libs/libvpx | WebM VP8 and VP9 Codec SDK |
| media-libs/libyuv | libyuv is an open source project that includes YUV scaling and   |
| media-libs/libzen | Shared library for libmediainfo and mediainfo |
| media-libs/materialx | MaterialX is an open standard for the exchange of rich material   |
...
| media-libs/openpgl | Generated headers and sources for OpenXR loader. |
...
| media-libs/theoraplay | TheoraPlay is a simple library to make decoding of Ogg Theora   |
...
| media-plugins/RadeonProRenderBlenderAddon | This hardware-agnostic rendering plug-in for Blender uses   |
...
| media-plugins/gst-plugins-analyticsoverlay | analyticsoverlay plugin can highlight detected objects for GStreamer |
...
| media-sound/w3crapcli-lastfm | w3crapcli/last.fm provides a command line interface for the   |
...
| net-libs/cef-bin | Chromium Embedded Framework (CEF). A simple framework for   |
...
| sci-libs/MIVisionX | MIVisionX toolkit is a set of comprehensive computer vision and   |
...
| sci-libs/hipBLASLt | hipBLASLt is a library that provides general matrix-matrix   |
...
| sci-libs/intel-extension-for-pytorch | A Python package for extending the official PyTorch that can   |
...
| sci-libs/rocAL | The AMD rocAL is designed to efficiently decode and process   |
...
| sci-libs/rocWMMA | AMD's C++ library for accelerating mixed-precision matrix   |
| sci-libs/rpp | AMD ROCm Performance Primitives (RPP) library is a comprehensive   |
| sci-libs/tensorflow | Computation framework using data flow graphs for scalable machine   |
...
| sci-physics/mujoco | Multi-Joint dynamics with Contact. A general purpose physics   |
...
| sys-apps/c2tcp | C2TCP: A Flexible Cellular TCP to Meet Stringent Delay   |
...
| sys-apps/deepcc | DeepCC: A Deep Reinforcement Learning Plug-in to Boost the   |
...
| sys-apps/oomd | oomd is userspace Out-Of-Memory (OOM) killer for linux systems. |
...
| sys-cluster/rdc | The ROCm™ Data Center Tool simplifies the administration and   |
| sys-cluster/ucx | Unified Communication X |
| sys-cluster/xpmem | XPMEM is a Linux kernel module that enables a process to map the   |
...
| www-misc/socli | Stack overflow command line client. Search and browse stack   |
...
| x11-wm/xpra | X Persistent Remote Apps (xpra) and Partitioning WM (parti) based   |
