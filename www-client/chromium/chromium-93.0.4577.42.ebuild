...
init_third_party_packages() {
	...
	# See also https://github.com/chromium/chromium/tree/93.0.4577.42/build/linux/unbundle
	THIRD_PARTY_PACKAGES=(
		dev-libs/expat
		dev-libs/libxml2
		media-libs/flac
...
check_dependencies_built_with_ssp() {
	...
	# https://github.com/chromium/chromium/blob/93.0.4577.42/build/config/compiler/BUILD.gn#L1677
...
check_dependencies_built_with_noexecstack() {
	# See https://github.com/chromium/chromium/blob/93.0.4577.42/build/config/compiler/BUILD.gn#L441
...
check_dependencies_built_with_full_relro() {
	# See https://github.com/chromium/chromium/blob/93.0.4577.42/build/config/compiler/BUILD.gn#L443
	...
	if use pgo-full ; then
		...
		if use cr_pgo_trainers_media_desktop ; then
			...
			if use vaapi && vainfo 2>/dev/null \
				...
				if (( ${ncpus} == 1 )) ; then
					av1_encoding=( -c:v libaom-av1 -cpu-used 8 -tile-columns 0 -tile-rows 0 -threads 1 )
				elif (( ${ncpus} == 2 && ${nthreads_per_core} > 1 )) ; then
					av1_encoding=( -c:v libaom-av1 -cpu-used 8 -tile-columns 0 -tile-rows 1 -threads $(( ${ncpus} * ${nthreads_per_core} )) -row-mt 1 )
				elif (( ${ncpus} == 2 )) ; then
					av1_encoding=( -c:v libaom-av1 -cpu-used 8 -tile-columns 0 -tile-rows 1 -threads ${ncpus} )
				elif (( ${ncpus} == 3 )) ; then
					av1_encoding=( -c:v libaom-av1 -cpu-used 8 -tile-columns 0 -tile-rows 1 -threads ${ncpus} -row-mt 1 )
				elif (( ${ncpus} == 4 && ${nthreads_per_core} > 1 )) ; then
					av1_encoding=( -c:v libaom-av1 -cpu-used 8 -tile-columns 1 -tile-rows 1 -threads $(( ${ncpus} * ${nthreads_per_core} )) -row-mt 1 )
				elif (( ${ncpus} == 4 )) ; then
					av1_encoding=( -c:v libaom-av1 -cpu-used 8 -tile-columns 1 -tile-rows 1 -threads ${ncpus} )
				elif (( ${ncpus} == 6 )) ; then
					av1_encoding=( -c:v libaom-av1 -cpu-used 8 -tile-columns 1 -tile-rows 1 -threads ${ncpus} -row-mt 1 )
				elif (( ${ncpus} == 8 && ${nthreads_per_core} > 1 )) ; then
					av1_encoding=( -c:v libaom-av1 -cpu-used 8 -tile-columns 1 -tile-rows 2 -threads $(( ${ncpus} * ${nthreads_per_core} )) -row-mt 1 )
				elif (( ${ncpus} == 8 )) ; then
					av1_encoding=( -c:v libaom-av1 -cpu-used 8 -tile-columns 1 -tile-rows 2 -threads ${ncpus} )
				elif (( ${ncpus} == 16 && ${nthreads_per_core} > 1 )) ; then
					av1_encoding=( -c:v libaom-av1 -cpu-used 8 -tile-columns 2 -tile-rows 2 -threads $(( ${ncpus} * ${nthreads_per_core} )) -row-mt 1 )
				elif (( ${ncpus} == 16 )) ; then
					av1_encoding=( -c:v libaom-av1 -cpu-used 8 -tile-columns 2 -tile-rows 2 -threads ${ncpus} )
				elif (( ${nthreads_per_core} > 1 )) ; then
					av1_encoding=( -c:v libaom-av1 -cpu-used 8 -tile-columns 2 -tile-rows 2 -threads $(( ${ncpus} * ${nthreads_per_core} )) -row-mt 1 )
				else
					av1_encoding=( -c:v libaom-av1 -cpu-used 8 -tile-columns 2 -tile-rows 2 -threads ${ncpus} -row-mt 1 )
				fi
