...
COMMON_DEPEND="
	...
	system-ffmpeg? ( >=media-video/ffmpeg-2.7.2:=[${MULTILIB_USEDEP}] media-libs/opus:=[${MULTILIB_USEDEP}] )
	sys-apps/dbus:=
...
multilib_src_configure() {
	...
	if [[ $myarch = amd64 ]] ; then
		...
		#workaround for row_gcc.cc: error: 'asm' operand has impossible constraints
		replace-flags "-Os" "-O2"
