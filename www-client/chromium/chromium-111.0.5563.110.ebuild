...
#
LICENSE_FINGERPRINT="\
869eea7c60be46845b24965cbb79d7b05bdf5ec52f1d54efe0044e4f1e547da5\
873304a01c7f85e5bba4cab2937d805d1061de8b70ab06e36f2d893da9e9c99e\
"
LICENSE="
	BSD
...
#   https://github.com/chromium/chromium/blob/111.0.5563.110/sandbox/linux/BUILD.gn
#
CPU_FLAGS_ARM=(
	neon
)
...
#
#   https://github.com/chromium/chromium/blob/111.0.5563.110/media/filters/BUILD.gn#L160
#   https://github.com/chromium/chromium/blob/111.0.5563.110/media/media_options.gni#L38
#   https://github.com/chromium/chromium/blob/111.0.5563.110/media/base/supported_types.cc#L203
#   https://github.com/chromium/chromium/blob/111.0.5563.110/media/base/supported_types.cc#L284
#
# Codec upstream default:
#   https://github.com/chromium/chromium/blob/111.0.5563.110/tools/mb/mb_config_expectations/chromium.linux.json#L89
#

#
# For cfi-vcall, cfi-icall defaults status, see \
#   https://github.com/chromium/chromium/blob/111.0.5563.110/build/config/sanitizers/sanitizers.gni
# For cfi-cast default status, see \
#   https://github.com/chromium/chromium/blob/111.0.5563.110/build/config/sanitizers/sanitizers.gni#L123
# For pgo default status, see \
#   https://github.com/chromium/chromium/blob/111.0.5563.110/build/config/compiler/pgo/pgo.gni#L15
# For libcxx default, see \
#   https://github.com/chromium/chromium/blob/111.0.5563.110/build/config/c++/c++.gni#L14
...
#   https://github.com/chromium/chromium/blob/111.0.5563.110/tools/clang/scripts/update.py#L42
...
pre_build_checks() {
	...
	if use official ; then
	# https://github.com/chromium/chromium/blob/111.0.5563.110/docs/linux/build_instructions.md#system-requirements
...
# LLVM 16
CR_CLANG_USED="39da55e8" # Obtained from \
# https://github.com/chromium/chromium/blob/111.0.5563.110/tools/clang/scripts/update.py#L42 \
# https://github.com/llvm/llvm-project/commit/87d0ff91
CR_CLANG_USED_UNIX_TIMESTAMP="1674023516" # Cached.  Use below to obtain this. \
...
ewarn
		...
		# Breaks during applying
		rm -rf "${WORKDIR}/patches/chromium-110-dpf-arm64.patch" || die
		rm -rf "${WORKDIR}/patches/chromium-111-v8-std-layout1.patch" || die
		rm -rf "${WORKDIR}/patches/chromium-111-v8-std-layout2.patch" || die
	fi
...
einfo
	...
	# is_debug=false.
	#
	# See https://github.com/chromium/chromium/blob/111.0.5563.110/build/config/compiler/compiler.gni#L276
	#
...
ewarn
	...
	# See https://github.com/chromium/chromium/blob/111.0.5563.110/media/media_options.gni#L19
...
ewarn
	...
	# See https://github.com/chromium/chromium/blob/111.0.5563.110/build/config/sanitizers/BUILD.gn#L196
	# See https://github.com/chromium/chromium/blob/111.0.5563.110/tools/mb/mb_config.pyl#L2950
...
einfo
	...
	# See also https://github.com/chromium/chromium/blob/111.0.5563.110/docs/pgo.md
...
pkg_postinst() {
	...
	if ! use headless; then
		if use vaapi ; then
	# It says 3 args:
	# https://github.com/chromium/chromium/blob/111.0.5563.110/docs/gpu/vaapi.md#vaapi-on-linux
einfo
