...
CDEPEND="
	>=dev-libs/nss-3.44.3[${MULTILIB_USEDEP}]
	>=dev-libs/nspr-4.21[${MULTILIB_USEDEP}]
	...
	dbus? ( >=sys-apps/dbus-0.60[${MULTILIB_USEDEP}]
		>=dev-libs/dbus-glib-0.72[${MULTILIB_USEDEP}] )
	startup-notification? ( >=x11-libs/startup-notification-0.8 )
	>=x11-libs/pixman-0.19.2[${MULTILIB_USEDEP}]
	>=dev-libs/glib-2.26:2[${MULTILIB_USEDEP}]
	>=sys-libs/zlib-1.2.3[${MULTILIB_USEDEP}]
	>=virtual/libffi-3.0.10:=[${MULTILIB_USEDEP}]
	...
	system-harfbuzz? ( >=media-libs/harfbuzz-2.4.0:0=[${MULTILIB_USEDEP}] >=media-gfx/graphite2-1.3.13[${MULTILIB_USEDEP}] )
	system-icu? ( >=dev-libs/icu-63.1:=[${MULTILIB_USEDEP}] )
	...
	system-libvpx? ( =media-libs/libvpx-1.7*:0=[postproc,${MULTILIB_USEDEP}] )
	system-sqlite? ( >=dev-db/sqlite-3.28.0:3[secure-delete,debug=,${MULTILIB_USEDEP}] )
	system-webp? ( >=media-libs/libwebp-1.0.2:0=[${MULTILIB_USEDEP}] )
	wifi? ( kernel_linux? ( >=sys-apps/dbus-0.60[${MULTILIB_USEDEP}]
			>=dev-libs/dbus-glib-0.72[${MULTILIB_USEDEP}]
...
RDEPEND="${CDEPEND}
	...
	pulseaudio? ( || ( media-sound/pulseaudio[${MULTILIB_USEDEP}]
		>=media-sound/apulse-0.1.9[${MULTILIB_USEDEP}] ) )
	selinux? ( sec-policy/selinux-mozilla )"
...
DEPEND="${CDEPEND}
	...
	|| (
		...
		(
			sys-devel/clang:6[${MULTILIB_USEDEP}]
			!clang? ( sys-devel/llvm:6[${MULTILIB_USEDEP}] )
			clang? (
				=sys-devel/lld-6*
				sys-devel/llvm:6[gold,${MULTILIB_USEDEP}]
				pgo? ( =sys-libs/compiler-rt-sanitizers-6*[profile] )
			)
		)
	)
	pulseaudio? ( media-sound/pulseaudio[${MULTILIB_USEDEP}] )
	>=dev-lang/rust-1.34.0[${MULTILIB_USEDEP}]
	!dev-lang/rust-bin
...
multilib_src_configure() {
	...
	echo "export CROSS_COMPILE=1" >> .mozconfig
	mozconfig_annotate '' --target=${chost%-*}
