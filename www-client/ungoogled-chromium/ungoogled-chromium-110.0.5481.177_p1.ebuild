...
# This ebuild fork just adds mold linker support and removes or permenently
# disables vendored services as preconditions for mold.  Mold can only be used
...
LLVM_SLOTS=(16)
LLVM_MAX_SLOT=16 # For the prebuilt pgo profile
CHROMIUM_LANGS="
...
inherit check-linker cflags-depends flag-o-matic-om llvm
...
DESCRIPTION="Modifications to Chromium for removing vendor integration and enhancing privacy"
...
optimize-thinlto optimize-webui pgo pic pulseaudio qt5 screencast selinux suid
+system-av1 +system-ffmpeg +system-harfbuzz +system-icu +system-jsoncpp
+system-libevent +system-libusb system-libvpx system-openjpeg +system-png
+system-re2 +system-snappy thinlto vaapi wayland +X
"
RESTRICT="
"
...
ewarn
ewarn "You have sys-libs/libcxx, please be aware that system-* and some other"
ewarn "c++ dependencies need to be compiled with the same library as"
ewarn "ungoogled-chromium itself dev-libs/jsoncpp is most problematic, see #58"
ewarn "#49 #119 for details"
ewarn
	fi
	if use cfi; then
ewarn
...
ewarn
ewarn "New vaapi code depends heavily on libvpx, see #43.  Consider disabling"
ewarn "system-libvpx USE flag if using vaapi.  A patch to make vaapi compatible"
ewarn "with system libvpx is welcomed."
ewarn
...
src_prepare() {
	...
	#if use epgo ; then
	#	eapply "${FILESDIR}/extra-patches/chromium-104.0.5112.79-gcc-pgo-link-gcov.patch"
	#fi
	...
	if use arm64 && has_sanitizer_option "shadow-call-stack" ; then
		eapply "${FILESDIR}/extra-patches/chromium-94-arm64-shadow-call-stack.patch"
	fi

	eapply "${FILESDIR}/extra-patches/chromium-103.0.5060.53-zlib-selective-simd.patch"

	eapply "${FILESDIR}/extra-patches/chromium-110-mold.patch"
...
ewarn "Keeping binary compiler.jar in source tree for js-type-check"
		sed -i '\!third_party/closure_compiler/compiler/compiler.jar!d' \
			"${ugc_pruning_list}" || die
	fi
...
ewarn "No substitutions in ${p}"
	...
	ebegin "Applying domain substitution"
		"${UGC_WD}/utils/domain_substitution.py" \
			-q apply \
			-r "${UGC_WD}/domain_regex.list" \
			-f "${UGC_WD}/domain_substitution.list" \
	...
	# It requires git and clang, bug #832803
			sed -i \
				-e "s|^update_readme||g" \
...
ewarn "Only the latest commit is supported for clang:${CR_CLANG_SLOT_OFFICIAL}"
			CC=${CHOST}-clang-${CR_CLANG_SLOT_OFFICIAL}
			CXX=${CHOST}-clang++-${CR_CLANG_SLOT_OFFICIAL}
		fi
...
einfo "Enforcing the use of gcc due to USE=-clang ..."
	...
	if use thinlto || use pgo; then
		if use mold ; then
eerror "LLD and mold are mutually exclusive.  Choose only one."
			die
		fi
...
ewarn "Using mold may weaken security"
	...
	myconf_gn+=" enable_hangout_services_extension=false" # This is their IP
	myconf_gn+=" enable_widevine=false" # This is their IP
	...
	myconf_gn+=" enable_platform_hevc=false"
	myconf_gn+=" enable_hevc_parser_and_hw_decoder=false"

	# Ungoogled flags
	myconf_gn+=" enable_mdns=false"
	myconf_gn+=" enable_mse_mpeg2ts_stream_parser=false"
	...
	# Prevent libvpx/xnnpack build failures. Bug 530248, 544702, 546984,
	# 853646.
	...
	# on speedometer when changing import-instr-limit from 100 to
	# 30.
	...
	if use headless; then
		...
		if use qt5; then
			...
			if tc-is-cross-compiler; then
	# A hack to workaround get_libdir not being able to handle CBUILD,
	# bug #794181
...
src_install() {
	...
	if  has_version ">=media-sound/apulse-0.1.9" ; then
		sed -i 's/exec -a "chromium-browser"/exec -a "chromium-browser" apulse/' \
	...
	# I kept the old symlink around for consistency.
...
einfo "by adding --enable-features=VaapiVideoDecoder and "
einfo "--disable-features=UseChromeOSDirectVideoDecoder to CHROMIUM_FLAGS"
...
einfo "behaviour you need to pass --gtk-version=4, e.g. by adding it"
einfo "to CHROMIUM_FLAGS in /etc/chromium/default."
einfo
		fi
	fi
