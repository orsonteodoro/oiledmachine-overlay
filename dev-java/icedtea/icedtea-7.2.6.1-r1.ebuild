...
IUSE="+awt +alsa cacao cjk +cups debug doc examples +gtk infinality
	...
	pulseaudio sctp selinux smartcard source +sunec test zero +webstart shark"
...
src_configure() {
	...
	if use shark && "x${use_zero}" = "xyes"; then
		shark_config="--enable-shark"
	elif use shark && "x${use_zero}" != "xyes"; then
		die "Enable the zero use flag."
	fi
	...
	econf ${config} \
		...
		$(use_enable sunec) \
		${zero_config} ${cacao_config} ${jamvm_config} ${shark_config}
}
