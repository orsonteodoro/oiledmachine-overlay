...
if [[ "${PV}" == "9999" && "${VCS}" == "git" ]] ; then
	FALLBACK_COMMIT="1eca07ed7386da8e8b927aff393f7836b5de64ba"
	EGIT_BRANCH="master"
...
eerror
eerror "Network activity disabled using EVCS_OFFLINE and clone of repository"
eerror "missing: \"${distdir}/fossil-src/${repo_id}/${repo_id}.fossil\""
eerror
			die
		fi
	else
...
_src_compile() {
	...
	if [[ ${PV} == 9999 ]] && use doc && multilib_is_native_abi ; then
		...
		emake -f "../${PN}-doc/Makefile" \
			...
			CC="$(tc-getBUILD_CC)" \
			TCLINC="" \
			TCLFLAGS="$($(tc-getBUILD_PKG_CONFIG) --libs tcl) -ldl -lm" \
			base \
			doc
