...
intel-ocl lts no-repacks +opencl opencl_rocm opencl_orca opencl_pal opengl_mesa
...
# GRADLE_PKGS_UNPACK listed from:
...
#	"com.squareup.okio/okio/2.6.0/f06923d428f3c8e6f571043ec29a45d0cd9d2bf/okio-jvm-2.6.0.jar"
...
# Duplicated again because of sandbox restrictions with sed/expr/cut
# Converted above URIs


# obtained with gradlew -d
# grep -r -e "Performing HTTP GET" ${T}/build.log | cut -f 7- -d " "
...
https://repo.maven.apache.org/maven2/net/java/dev/jna/jna/5.0.0/jna-5.0.0.jar
https://repo.maven.apache.org/maven2/net/java/dev/jna/jna/5.0.0/jna-5.0.0.pom
https://repo.maven.apache.org/maven2/net/java/dev/jna/jna-platform/5.0.0/jna-platform-5.0.0.jar
https://repo.maven.apache.org/maven2/net/java/dev/jna/jna-platform/5.0.0/jna-platform-5.0.0.pom
...
compile_with_gradlew()
{
	...
	einfo "PATH=${PATH}"
	# Add the -d flag to inspect log for downloads
