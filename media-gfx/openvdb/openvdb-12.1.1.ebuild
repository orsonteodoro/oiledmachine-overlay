...
+nanovdb -numpy +libc-malloc -python +static-libs -tbbmalloc
...
eerror
	...
	clang --version || die
	use clang || die "Turn on the clang USE flag"
}

check_gcc() {
	use gcc || die "Turn on the gcc USE flag"
}

check_icc() {
	which icc || die "icc is not in PATH.  Configure PATH using per-package environment variables."
ewarn "You must set up CC/CXX manually for ICC support."
}
