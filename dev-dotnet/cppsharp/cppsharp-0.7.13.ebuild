...
REQUIRED_USE="|| ( ${USE_DOTNET} ) abi_x86_64? ( !abi_x86_32 ) gac? ( net45 ) !gac"
DEPEND="${RDEPEND}
	>=dev-util/premake-5.0.0_alpha10
	>=sys-devel/clang-3.9.0
	>=sys-devel/llvm-3.9.0[codegen]
	 !sys-devel/llvm:0/9999"
inherit dotnet eutils flag-o-matic mono multilib-minimal toolchain-funcs
SRC_URI=\
"https://github.com/mono/${PROJECT_NAME}/archive/${PV}.tar.gz \
	-> ${P}.tar.gz"
inherit gac
RESTRICT="mirror"
...
src_prepare() {
	...
	multilib_foreach_abi ml_prepare1
...
/usr/lib/clang/$(clang-fullversion)/include|g" \
				src/Generator.Tests/GeneratorTest.cs || die

			pushd build || die
			  #echo "c1316b6adfbb17b961a3bee357e728ca0d4d1c96" \
			  #  > LLVM-commit
				sed -i -e 's|\
path.join(LLVMRootDirDebug, "include"),|\
path.join("/usr/lib/clang/3.9.0/include/"),path.join("/usr/include/llvm"),\
path.join("/usr/include/clang"),path.join(LLVMRootDirDebug, "include"),|g' \
	LLVM.lua || die
				sed -i -e 's|\
path.join(LLVMRootDirRelease, "include"),|\
path.join("/usr/lib/clang/3.9.0/include/"),path.join("/usr/include/llvm"),\
path.join("/usr/include/clang"),path.join(LLVMRootDirRelease, "include"),|g' \
	LLVM.lua || die
				sed -i -e 's|\
path.join(LLVMRootDir, "include"),|\
path.join("/usr/lib/clang/3.9.0/include/"),path.join("/usr/include/llvm"),\
path.join("/usr/include/clang"),path.join(LLVMRootDir, "include"),|g' \
	LLVM.lua || die
				sed -i -e 's|\
error("Error finding an LLVM build")|\
--error("Error finding an LLVM build")|g' \
	LLVM.lua || die
...
ALL_CXXFLAGS += \$(CXXFLAGS) \$(ALL_CFLAGS) -fno-rtti -fPIC|g' \
	gmake/projects/CppSharp.CppParser.make || die
#				sed -i -r -e ':a' -e 'N' -e '$!ba' -e 's|\
#ALL_LDFLAGS \+= \$\(LDFLAGS\) -L\/usr\/lib64 -L..\/..\/..\/deps\/llvm\/build\/lib -m64 -shared|\
#ALL_LDFLAGS += \$(LDFLAGS) -L\/usr\/lib64 -L..\/..\/..\/deps\/llvm\/build\/lib -m64 -shared -fPIC|g' \
# gmake/projects/CppSharp.CppParser.make || die
#				sed -i -r -e ':a' -e 'N' -e '$!ba' -e 's|\
#ALL_LDFLAGS \+= \$\(LDFLAGS\) -L\/usr\/lib32 -L..\/..\/..\/deps\/llvm\/build\/lib -m32 -shared|\
#ALL_LDFLAGS += \$(LDFLAGS) -L\/usr\/lib32 -L..\/..\/..\/deps\/llvm\/build\/lib -m32 -shared -fPIC|g' \
# gmake/projects/CppSharp.CppParser.make || die
			popd
...
options.addIncludeDirs(path);\
options.addSystemIncludeDirs(\"\/usr\/lib\/clang\/$(ls /usr/lib/clang/)\/include\/\");|g" \
			src/Generator.Tests/GeneratorTest.cs || die
			sed -i -e "s|\
-I/usr/lib/clang/3.9.0/include|\
-I/usr/lib/clang/$(clang-fullversion)/include|g" \
			...
			do
				...
				sed -i -r -e "s|\
FLAGS = /unsafe|\
FLAGS = -sdk:${EBF} -keyfile:\"${DISTDIR}/mono.snk\" /unsafe|g" \
					"$f" || die
			done
			...
			do
				...
				sed -i -r -e "s|\
/nologo|\
-sdk:${EBF} -keyfile:\"${DISTDIR}/mono.snk\" /nologo|g" \
					"$f" || die
			done
...
if use gac ; then
			#inject public key into assembly
		        public_key=\
	$(sn -tp "${DISTDIR}/mono.snk" | tail -n 7 | head -n 5 | tr -d '\n')
		        echo "pk is: ${public_key}"
			FILES=$(grep -l -r -e "InternalsVisibleTo" ./)
			for f in $FILES
			do
				einfo "Attempting to inject public key into $f"
				estrong_assembly_info2 "CppSharp.Parser" \
					"${DISTDIR}/mono.snk" "$f"
				estrong_assembly_info2 "CppSharp.Parser.CSharp" \
					"${DISTDIR}/mono.snk" "$f"
			done
