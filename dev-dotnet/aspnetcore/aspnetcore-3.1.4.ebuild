...
SRC_URI_TGZ="https://github.com/aspnet/AspNetCore/archive/v${PV}.tar.gz -> ${PN}-${PV}.tar.gz
	     ...
	     https://github.com/aspnet/MessagePack-CSharp/archive/${MESSAGEPACK_CSHARP_COMMIT}.tar.gz -> messagepack-csharp-${MESSAGEPACK_CSHARP_COMMIT}.tar.gz"
SRC_URI_TGZ=""
fi
# We need to cache the dotnet-sdk and dotnet-runtime tarballs outside the sandbox otherwise we have to keep downloading it everytime the sandbox is wiped.
SRC_URI="${SRC_URI_TGZ}
	 amd64? ( https://dotnetcli.azureedge.net/dotnet/Sdk/${SDK_V}/dotnet-sdk-${SDK_V}-linux-x64.tar.gz
	 https://dotnetcli.azureedge.net/dotnet/Runtime/${NETCORE_V}/dotnet-runtime-${NETCORE_V}-linux-x64.tar.gz )"
...
_unpack_asp() {
	...
	pushd src/submodules || die
	mv "${WORKDIR}/MessagePack-CSharp-${MESSAGEPACK_CSHARP_COMMIT}"/ "${WORKDIR}/MessagePack-CSharp" || die
...
#	X_NETCORE_V=$(grep -r -e "\.SDK" scripts/VsRequirements/vs.json | cut -f 2 -d "\"" | sed  -r -e "s|.*([0-9]+.[0-9]+.[0-9]+).*|\1|g")
#	if [[ ! -f scripts/VsRequirements/vs.json ]] ; then
#		die "Cannot find scripts/VsRequirements/vs.json"
#	elif [[ "${X_NETFX_V}" != "${NETFX_V}" ]] ; then
#		die "Cached dotnet-runtime in distfiles is not the same as requested.  Update ebuild's NETCORE_V to ${X_NETCORE_V}"
#	fi
...
_use_ms_netcore() {
	...
	mkdir -p "${p}" || die
	pushd "${p}" || die
		tar -xvf "${DISTDIR}/dotnet-runtime-${NETCORE_V}-linux-${myarch}.tar.gz" || die
	popd || die
}
...
_src_compile() {
	...
	if [[ ${ARCH} =~ (amd64) ]]; then
		...
		cd "${ASPNETCORE_S}" || die
		./build.sh --verbosity normal --arch ${myarch} --configuration ${mydebug^} ${buildargs_coreasp} || die
	fi
}
...
src_install() {
	...
	cp -a src/SignalR/clients/ts/signalr/src/{,SignalR-clients-ts-}third-party-notices.txt
	dodoc src/SignalR/clients/ts/signalr/src/SignalR-clients-ts-third-party-notices.txt
	cp -a src/SignalR/clients/ts/signalr-protocol-msgpack/src/{,SignalR-clients-ts-signalr-protocol-msgpack-}third-party-notices.txt
	dodoc src/SignalR/clients/ts/signalr-protocol-msgpack/src/SignalR-clients-ts-signalr-protocol-msgpack-third-party-notices.txt
	...
	cp -a src/Components/benchmarkapps/BlazingPizza.Server/{,Components-benchmarkapps-BlazingPizza.Server-}THIRD-PARTY-NOTICES.md
	dodoc src/Components/benchmarkapps/BlazingPizza.Server/Components-benchmarkapps-BlazingPizza.Server-THIRD-PARTY-NOTICES.md
