...
_src_prepare() {
	cp "${FILESDIR}/corefx-${PATCH_V1}-buildtools.patch" "${COREFX_S}/buildtools.patch"
...
_src_compile() {
	...
	#cd "${COREFX_S}" || die
	#./build-native.sh -${myarch} -${mydebug} -verbose ${buildargs_corefx} || die
	#if use tests ; then
	#	einfo "Building CoreFX tests"
	#	cd "${COREFX_S}"
	#	./build-tests.sh -${myarch} -${mydebug} -verbose || die
	#fi
}
...
src_install() {
	...
	cp -a "${COREFX_S}/bin/Linux.${myarch}.Release/native"/* "${ddest_core}/${PV}/" || die
}
