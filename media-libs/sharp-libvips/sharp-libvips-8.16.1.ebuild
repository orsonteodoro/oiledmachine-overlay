...
src_install() {
	...
	if [ -d "${ED}/usr/lib/sharp-vips/${libdir}/pkgconfig" ]; then
		...
		sed -i '/Libs:/ s|$| -laom -lheif -lvips -lglib-2.0 -lgobject-2.0 -lgio-2.0 -lz -ljpeg -lpng -ltiff -lwebp -larchive -llcms2 -lfontconfig -lpangocairo-1.0 -lpangoft2-1.0 -lrsvg-2|' "${ED}/usr/lib/sharp-vips/${libdir}/pkgconfig/vips.pc" || die "Failed to add static libs to vips.pc"
