...
_process_one_lockfile() {
	local lockfile_path="${1}"
	local l

	echo "Processing ${lockfile_path}"
...
main() {
	IFS=$'\n'
	local lockfile_path
	for lockfile_path in $(find "/var/tmp/portage/${CATEGORY}/${PN}-${PV}/work" -name "Cargo.lock") ; do
		_process_one_lockfile "${lockfile_path}"
	done
	IFS=$' \n\r\t'
}
