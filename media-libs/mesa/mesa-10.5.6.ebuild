...
multilib_src_install() {
	...
	if use opencl; then
		...
		if [ -f "${ED}/usr/$(get_libdir)/libMesaOpenCL.so" ]; then
			mv -f "${ED}"/usr/$(get_libdir)/libMesaOpenCL.so* \
			"${ED}"${cl_dir}
			ln -s "${ED}"${cl_dir}"/libMesaOpenCL.so" "${ED}"${cl_dir}"/libOpenCL.so"
			cp -R "${WORKDIR}/${P}/include/CL" "${ED}"${cl_dir}/include
		fi
