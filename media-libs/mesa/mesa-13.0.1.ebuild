...
inherit eutils autotools multilib-minimal python-any-r1 pax-utils ${GIT_ECLASS} versionator toolchain-funcs
...
src_prepare() {
	#versionator is broken for clang-9999
	clang --version
	if [[ "$?" == "0" ]]; then
		if $(version_is_at_least "5.0.0" $(clang --version | head -n1 | grep -o "[0-9].[0-9].[0-9]")); then
			eapply "${FILESDIR}/mesa-13.0.1-fix-depricated-attribute.patch"
			eapply "${FILESDIR}/mesa-13.0.1-remove-readerwriter-header-1.patch"
			eapply "${FILESDIR}/mesa-13.0.1-remove-readerwriter-header-2.patch"
		fi
	fi
	eapply_user
	[[ ${PV} == 9999 ]] && eautoreconf
