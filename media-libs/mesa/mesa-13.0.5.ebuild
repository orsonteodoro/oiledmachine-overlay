...
inherit autotools multilib-minimal python-any-r1 pax-utils ${GIT_ECLASS} versionator toolchain-funcs
...
src_prepare() {
	...
	if [[ "$?" == "0" ]]; then
		...
		if $(version_is_at_least "4.0.0" $(clang --version | head -n1 | grep -o "[0-9].[0-9].[0-9]")); then
	...
	#eapply "${FILESDIR}/mesa-13.0.5-configure.ac-extract-HAVE_LLVM-macro-properly.patch"
