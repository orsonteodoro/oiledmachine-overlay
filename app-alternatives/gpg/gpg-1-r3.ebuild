...
ALTERNATIVES=(
	"reference:>=app-crypt/gnupg-${GNUPG_PV}:=[alternatives(-),nls?,ssl?]"
...
pkg_postinst() {
	use freepg && ewarn "SECURITY NOTICE:  The USE=freepg is not using latest security fixes from live.   USE AT YOUR OWN RISK."
	use sequoia && ewarn "SECURITY NOTICE:  The USE=sequoia is using the unsafe keyword without fuzzing scripts.  USE AT YOUR OWN RISK."
}
