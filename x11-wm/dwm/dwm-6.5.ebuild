...
src_prepare() {
	...
	if use mod_sizehintsoff ; then
		sed -i \
			-e "s|resizehints = 1|resizehints = 0|g" \
			"config.def.h" \
			|| die
	fi
...
einfo
	...
	if use mod_rotatestack ; then
einfo
einfo "mod_rotatestack requires tilling mode, fibonacci mode to be on."
einfo
...
# OILEDMACHINE-TEST:  passed (6.5) interactive
# mod_fibonacci - passed
# mod_rotatestack - passed
