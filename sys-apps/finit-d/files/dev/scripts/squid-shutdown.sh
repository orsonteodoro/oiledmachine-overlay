...
stop() {
	ebegin "Stopping squid with /usr/sbin/squid -k shutdown -f /etc/squid/squid.conf -n squid"
	if /usr/sbin/squid -k shutdown -f /etc/squid/squid.conf -n squid ; then
