...
einfo "Installing ${PN}"
		...
		if use cert-only-pem ; then
			cat \
				"${EROOT}/etc/${PN}/certs/${PN}.csr" \
				> \
				"${EROOT}/etc/${PN}/certs/${PN}.pem" \
				|| die
		else
ewarn
...
ewarn
		...
		chown "${PN}:${PN}" "${EROOT}/etc/${PN}/certs/${PN}."{"crt","csr","key","pem"} || die
		chmod 0644 "${EROOT}/etc/${PN}/certs/${PN}.pem" || die
		chmod 0640 "${EROOT}/etc/${PN}/certs/${PN}.key" || die
	fi
}
