...
DKMS_PKG_VER="${SUFFIX}"
DC_VER="3.2.212" # See https://github.com/RadeonOpenCompute/ROCK-Kernel-Driver/blob/rocm-5.4.3/drivers/gpu/drm/amd/display/dc/dc.h#L48
PATCHES=(
...
eerror "package.  For this kernel it is named ROCK_DKMS_KERNELS_5_4."
eerror
...
eerror
eerror "Kernel version ${kv} is not supported.  Update your ROCK_DKMS_KERNELS_5_4"
...
pkg_setup() {
	show_supported_kv
	if [[ -z "${ROCK_DKMS_KERNELS_5_4}" ]] ; then
eerror
eerror "You must define a per-package env or add to /etc/portage/make.conf an"
eerror "environmental variable named ROCK_DKMS_KERNELS_5_4 containing a space"
eerror "delimited <kernvel_ver>-<extra_version>."
eerror
eerror "It should look like ROCK_DKMS_KERNELS_5_4=\"${KV}-pf ${KV}-zen\""
eerror
		die
	fi
...
if [[ "${MAINTAINER_MODE}" != "1" ]] ; then
	local k
	for k in ${ROCK_DKMS_KERNELS_5_4} ; do
		if [[ "${k}" =~ "*" ]] ; then
...
# See also https://github.com/RadeonOpenCompute/ROCK-Kernel-Driver/blob/rocm-5.4.3/drivers/gpu/drm/amd/dkms/sources
...
set_cc() {
	...
	if [[ "${CC}" == "gcc" ]] && (( ${old_style} == 1 )) ; then
		local gcc_slot=$(grep "Compiler:" "/usr/src/linux-${k}/.config" \
			| cut -f 2- -d ":" \
			| sed -r -e "s|^[ ]+||g" \
	...
	elif [[ "${CC}" == "clang" ]] && (( ${old_style} == 1 )) ; then
		local clang_slot $(grep "Compiler:" "/usr/src/linux-${k}/.config" \
			| cut -f 2 -d ":" \
			| sed -r -e "s|^[ ]+||g" \
			| cut -f 3 -d " " \
			| cut -f 1 -d ".")
		export CC="${CHOST}-clang-${clang_slot}"
	elif [[ "${CC}" == "gcc" ]] && (( ${old_style} == 0 )) ; then
		local gcc_slot=$(grep -r -e "CONFIG_CC_VERSION_TEXT" "/usr/src/linux-${k}/.config" \
			| cut -f 2 -d '"' \
			| grep -E -o -e "[0-9]+\.[0-9]+\.[0-9]+" \
			| tail -n 1 \
			| cut -f 1 -d ".")
		export CC="${CHOST}-gcc-${gcc_slot}"
	elif [[ "${CC}" == "clang" ]] && (( ${old_style} == 0 )) ; then
		local gcc_slot=$(grep -r -e "CONFIG_CC_VERSION_TEXT" "/usr/src/linux-${k}/.config" \
			| cut -f 2 -d '"' \
			| grep -E -o -e "[0-9]+\.[0-9]+\.[0-9]+" \
			| tail -n 1 \
...
die_build() {
	local root_path
	if [[ "${USE_DKMS}" == "1" ]] ; then
		root_path="${dkms_build_path}/${DKMS_PKG_NAME}/${DKMS_PKG_VER}"
	else
		root_path="${build_root}"
	fi
	env > "${root_path}/build/env.log"
eerror
eerror "Log dumps:"
eerror
eerror "${root_path}/build/env.log"
eerror "${root_path}/build/make.log"
eerror
	die "${@}"
}
...
_build_clean() {
	if [[ "${USE_DKMS}" == "1" ]] ; then
		...
		[[ -e "/${DKMS_PKG_NAME}/${DKMS_PKG_VER}/bin" ]] && die
		rm -rf "/${DKMS_PKG_NAME}/${DKMS_PKG_VER}"
	else
		[[ -e "${build_root}/bin" ]] && die
		rm -rf "${build_root}"
	fi
}
...
_copy_modules() {
	# Keep in sync with https://github.com/RadeonOpenCompute/ROCK-Kernel-Driver/blob/rocm-5.4.3/drivers/gpu/drm/amd/dkms/dkms.conf
	IFS=$'\n'
...
einfo "CONFIG_GCC_VERSION:  ${CONFIG_GCC_VERSION}"
	...
	local kv="${k%%-*}"
	local build_root
	local dkms_build_path
	if ver_test ${kv} -ge 5.15 ; then
		dkms_build_path="/"
		args+=( --dkmstree "${dkms_build_path}" )
		build_root="/rock_build"
	else
		# "/" breaks with kernel version 5.4
		# Default M=/var/lib/dkms/amdgpu/5.4/build
		dkms_build_path="/"
		args+=( --dkmstree "${dkms_build_path}" )
#		mkdir -p "${dkms_build_path}"
		build_root="/rock_build"
	fi
	_build_clean

	local KARCH=$(tc-arch-kernel)
	local _k="${kernel_release}/${KARCH}"
	if [[ "${USE_DKMS}" == "1" ]] ; then
einfo "Running:  \`dkms build ${DKMS_PKG_NAME}/${DKMS_PKG_VER} -k ${_k} ${args[@]}\`"
		dkms build "${DKMS_PKG_NAME}/${DKMS_PKG_VER}" -k "${_k}" ${args[@]} || die_build
einfo "Running:  \`dkms install ${DKMS_PKG_NAME}/${DKMS_PKG_VER} -k ${_k} --force ${args[@]}\`"
