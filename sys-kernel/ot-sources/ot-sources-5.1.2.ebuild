...
function get_missing_rock_commits_list() {
	...
	index=1
	cat /dev/null > "${T}"/amd-staging-drm-next.commits.indexed
	while IFS= read -r l ; do
		echo $(printf "%06d" ${index})" ${l}" >> "${T}"/amd-staging-drm-next.commits.indexed
		index=$((${index} + 1))
	done < "${T}"/amd-staging-drm-next.commits

	cat "${T}"/amd-staging-drm-next.commits.indexed | cut -c 49- | sort > "${T}"/amd-staging-drm-next.summaries
	...
	index=1
	cat /dev/null > "${T}"/rock.commits.indexed
	while IFS= read -r l ; do
		echo $(printf "%06d" ${index})" ${l}" >> "${T}"/rock.commits.indexed
		index=$((${index} + 1))
	done < "${T}"/rock.commits

	cat "${T}"/rock.commits.indexed | cut -c 49- | sort > "${T}"/rock.summaries
...
function get_missing_rock_commits() {
	...
	einfo "Plucking commits"
	for l in ${L} ; do
		grep -F -e "${l}" "${T}/rock.commits.indexed" >> "${T}/rock.found"
	done
