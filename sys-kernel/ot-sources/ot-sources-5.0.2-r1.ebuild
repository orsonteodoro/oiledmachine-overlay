...
IUSE="-zentune +o3 muqss +pds +bfq bmq +amd amd-staging-drm-next cfs +graysky2 +ukms tresor tresor_aesni tresor_i686 tresor_x86_64 tresor_sysfs"
REQUIRED_USE="!ukms ^^ ( muqss pds cfs bmq ) tresor_sysfs? ( || ( tresor_i686 tresor_x86_64 tresor_aesni ) ) tresor? ( ^^ ( tresor_i686 tresor_x86_64 tresor_aesni ) ) tresor_i686? ( tresor ) tresor_x86_64? ( tresor ) tresor_aesni? ( tresor ) amd-staging-drm-next? ( amd )"
...
DESCRIPTION="Orson Teodoro's patchset containing UKMS, zen-tune, GraySky's GCC Patches, MUQSS CPU Scheduler, PDS CPU Scheduler, BMQ CPU Scheduler, Genpatches, BFQ updates, AMD kernel updates, TRESOR"
