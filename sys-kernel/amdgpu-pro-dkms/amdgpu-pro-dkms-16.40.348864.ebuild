...
pkg_nofetch() {
	...
	einfo "  - amdgpu-pro-${BUILD_VER}.tar.xz for Ubuntu 16.04"
...
src_prepare() {
	...
	pushd ./usr/src/amdgpu-pro-${BUILD_VER} > /dev/null
		epatch "${FILESDIR}"/0014a-Add-Gentoo-as-OS-option.patch
		# Dont copy the firmware
		head -n -4 ./pre-build.sh > ./pre-build.sh.new
		mv ./pre-build.sh.new ./pre-build.sh
		chmod +x ./pre-build.sh

		epatch "${FILESDIR}/amdgpu-pro-16.40.348864-linux-4.9.6-compat.patch"
	popd > /dev/null
