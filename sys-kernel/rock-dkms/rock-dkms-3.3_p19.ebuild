...
src_install() {
	...
	doins -r "${S}"/*
	fperms 0770 /usr/src/${DKMS_PKG_NAME}-${DKMS_PKG_VER}/{post-install.sh,post-remove.sh,pre-build.sh,config/install-sh,configure,amd/dkms/configure,amd/dkms/pre-build.sh,autogen.sh,amd/dkms/autogen.sh}
...
pkg_postinst() {
	...
	if use build ; then
		for k in ${ROCK_DKMS_KERNELS} ; do
			if [[ "${k}" =~ "*" ]] ; then
				# pick all point releases: 5.2^-ot
				V=$(find /usr/src/ -maxdepth -name "linux-${k}" | sort | cut -f 4 -d "/" | sed -e "s|linux-||")
				for v in ${V} ; do
					k="${v}"
					dkms_build
				done
