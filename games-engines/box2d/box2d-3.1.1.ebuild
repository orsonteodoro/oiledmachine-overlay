...
${CPU_FLAGS_ARM[@]}
${CPU_FLAGS_X86[@]}
benchmarks doc samples static-libs test
ebuild_revision_5
"
REQUIRED_USE+="
	!ebolt
...
einfo "DISPLAY=${display}"
	if ( use pgo || use bolt ) && [[ -z "${display}" ]] ; then
...
einfo "Detected compiler switch.  Disabling LTO."
	...
	local mycmakeargs=(
		-DBOX2D_BENCHMARKS=$(usex benchmarks)
		-DBOX2D_DOCS=$(usex doc)
		-DBOX2D_UNIT_TESTS=$(usex test)
		-DBOX2D_SAMPLES=$(usex samples)
	)

	if [[ "${ARCH}" =~ ("amd64"|"x86") ]] ; then
		mycmakeargs+=(
			-DBOX2D_DISABLE_SIMD=$(usex cpu_flags_x86_sse2)
			-DBOX2D_AVX2=$(usex cpu_flags_x86_avx)
		)
	fi
	if [[ "${ARCH}" =~ ("arm64"|"arm"$) ]] ; then
		mycmakeargs+=(
			-DBOX2D_DISABLE_SIMD=$(usex cpu_flags_arm_neon)
		)
	fi
...
eerror "No unit test exist for ABI=${ABI} lib_type=${lib_type}"
				die
			fi
		done
	}
