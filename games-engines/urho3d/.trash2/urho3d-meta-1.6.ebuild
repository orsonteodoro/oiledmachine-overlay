...
IUSE="system-network boost sound alsa pulseaudio sse debug automated-testing javascript static static-libs +pch -docs pulseaudio -angelscript +lua -lua-cxx-wrapper-safety-checks -lua-jit -network -odbc sqlite +navigation +2d-physics +3d-physics +opengl +samples -extras +tools -clang-tools -raw-script-loader +filewatcher -cxx11 -bindings logging profiling threads debug native raspberry-pi android multitarget"
