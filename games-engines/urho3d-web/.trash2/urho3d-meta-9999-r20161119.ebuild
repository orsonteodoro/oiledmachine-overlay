...
IUSE="boost sound alsa pulseaudio sse debug automated-testing javascript static static-libs +pch -docs pulseaudio -angelscript +lua -lua-cxx-wrapper-safety-checks -lua-jit -network -odbc sqlite +navigation +2d-physics +3d-physics +opengl +samples -extras +tools -clang-tools -raw-script-loader +filewatcher -cxx11 -bindings logging profiling threads debug native raspberry-pi android multitarget"
...
src_unpack() {
        ...
        EGIT_BRANCH="master"
        EGIT_COMMIT="c7a63ac03e6845826840aa58d1b5c68c6c959e7d"
        git-r3_fetch
        git-r3_checkout
}
...
if $(false); then #knet fixes
	sed -i -e 's|network_ = new kNet::Network();|started = false;\nnetwork_ = new kNet::Network();|' Source/Urho3D/Network/Network.cpp || die p95
	sed -i -e 's|URHO3D_LOGINFO("Started server|started = true;\nURHO3D_LOGINFO("Started server|' Source/Urho3D/Network/Network.cpp || die p96
...
if use network; then
if use boost; then
	sed -i -e 's|# Add definitions for Emscripten|add_definitions (-DUNIX=1)\nadd_definitions (-DKNET_UNIX)\nadd_definitions (-DKNET_USE_BOOST=1)\n# Add definitions for Emscripten|' CMake/Modules/Urho3D-CMake-common.cmake || die p100
else
	sed -i -e 's|# Add definitions for Emscripten|add_definitions (-DUNIX=1)\nadd_definitions (-DKNET_UNIX)\n# Add definitions for Emscripten|' CMake/Modules/Urho3D-CMake-common.cmake || die p100
fi
fi
...
if use odbc; then
	if use static; then
		sed -i -e "s|set (TARGET_NAME Urho3D)|set (TARGET_NAME Urho3D)\nset (CMAKE_STATIC_LINKER_FLAGS \"\${CMAKE_STATIC_LINKER_FLAGS} -Qunused-arguments -I/usr/include/c++/v1 -DBOOST_TEST_DYN_LINK\")|" Source/Urho3D/CMakeLists.txt
	else
		sed -i -e "s|set (TARGET_NAME Urho3D)|set (TARGET_NAME Urho3D)\nset (CMAKE_SHARED_LINKER_FLAGS \"\${CMAKE_SHARED_LINKER_FLAGS} -Qunused-arguments -I/usr/include/c++/v1 -DBOOST_TEST_DYN_LINK\")|" Source/Urho3D/CMakeLists.txt
	fi
	sed -i -e 's|set (TARGET_NAME Urho3D)|set (TARGET_NAME Urho3D)\nset (CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -Qunused-arguments -I/usr/include/c++/v1 -DBOOST_TEST_DYN_LINK")|' Source/Urho3D/CMakeLists.txt
	sed -i -e 's|set (TARGET_NAME Urho3D)|set (TARGET_NAME Urho3D)\nset (CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -Qunused-arguments -I/usr/include/c++/v1 -DBOOST_TEST_DYN_LINK")|' Source/Urho3D/CMakeLists.txt
fi

	epatch "${FILESDIR}"/urho3d-9999-r20161119-tolua.patch
	...
	#epatch "${FILESDIR}"/urho3d-1.6-opassign-27edits.patch #omit
	...
	epatch "${FILESDIR}"/urho3d-9999-r20161119-graphics-windowicon.patch
	epatch "${FILESDIR}"/urho3d-graphics-getwindowicon.patch
	epatch "${FILESDIR}"/urho3d-9999-r20161119-oglgraphics_cpp-sdl_createwindowfrom-vanilla.patch

	epatch "${FILESDIR}"/urho3d-9999-r20161119-getautoremovemode.patch

	epatch "${FILESDIR}"/urho3d-9999-r20161119-debug-hud-getdefaultstyle.patch
	epatch "${FILESDIR}"/urho3d-1.6-xmlfile-constructor-1.patch
	epatch "${FILESDIR}"/urho3d-9999-r20161119-soundsource_pkg.patch
