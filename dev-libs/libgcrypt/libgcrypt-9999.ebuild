...
CFLAGS_HARDENED_VULNERABILITY_HISTORY="DOS HO II SC TA"

VERIFY_SIG_OPENPGP_KEY_PATH=/usr/share/openpgp-keys/gnupg.asc
inherit autotools cflags-hardened flag-o-matic linux-info multilib-minimal toolchain-funcs secure-version verify-sig

if [[ "${PV}" =~ "9999" ]] ; then
	FALLBACK_COMMIT="db5ae94eb01feb3cb60d42e6caa75106e3e781fc"
	EGIT_BRANCH="master"
	#EGIT_REPO_URI="https://dev.gnupg.org/source/gnupg.git" # Down for maintenance
...
SOVER="20"
SLOT="0/${SOVER}" # subslot = soname major version
...
RDEPEND="
	...
	elibc_glibc? ( >=sys-libs/glibc-${GLIBC_PV} )
...
src_unpack() {
	...
	local c=$(grep "^LIBGCRYPT_LT_CURRENT" "${S}/configure.ac" | cut -f 2 -d "=")
	local a=$(grep "^LIBGCRYPT_LT_AGE" "${S}/configure.ac" | cut -f 2 -d "=")
	local actual_sover=$(( ${c} - ${a} ))
	local expected_sover="${SOVER}"
	if ver_test "${actual_sover}" "-ne" "${expected_sover}" ; then
eerror "QA:  Bump SOVER in the ebuild."
