...
if [[ ${PV} == 9999 ]] ; then
	FALLBACK_COMMIT="64e89606ff4967a647525524aec00dd33765ba2a"
	EGIT_BRANCH="main"
	#EGIT_REPO_URI="https://sourceware.org/git/elfutils.git" # Too many requests
	EGIT_REPO_URI="https://forge.sourceware.org/elfutils/elfutils-mirror.git" # Mirror
...
RDEPEND="
	!dev-libs/libelf
	>=virtual/zlib-${ZLIB_PV}:=[static-libs?,${MULTILIB_USEDEP}]
	bzip2? ( >=app-arch/bzip2-${BZIP2_PV}:=[static-libs?,${MULTILIB_USEDEP}] )
	debuginfod? (
		>=dev-db/sqlite-${SQLITE_PV}:=
		>=dev-libs/json-c-${JSON_C_PV}:=
		>=net-libs/libmicrohttpd-${LIBMICROHTTPD_PV}:=
		>=net-misc/curl-${CURL_PV}:=[static-libs?]
	)
	libarchive? ( >=app-arch/libarchive-${LIBARCHIVE_PV}:= )
	lzma? ( >=app-arch/xz-utils-${XZ_UTILS_PV}:=[static-libs?,${MULTILIB_USEDEP}] )
	stacktrace? ( dev-util/sysprof:= )
	zstd? ( >=app-arch/zstd-${ZSTD_PV}:=[static-libs?,${MULTILIB_USEDEP}] )
	elibc_musl? (
