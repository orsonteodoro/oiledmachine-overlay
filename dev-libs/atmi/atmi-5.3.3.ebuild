...
"
PATCHES=(
	"${FILESDIR}/atmi-5.3.3-cmake_library_hint.patch"
)
S="${WORKDIR}/atmi-rocm-${PV}/src"

src_prepare() {
	sed \
		-e "s:DESTINATION lib COMPONENT device_runtime:DESTINATION lib64 COMPONENT device_runtime:" \
		-i \
		"${S}/device_runtime/CMakeLists.txt" \
		|| die
	sed \
		-e "s:TARGETS atmi_runtime LIBRARY DESTINATION \"lib\" COMPONENT runtime:TARGETS atmi_runtime LIBRARY DESTINATION \"lib64\" COMPONENT runtime:" \
		-i \
	...
	sed \
		-i \
		-e "s|-target|--rocm-device-lib-path=\"${ESYSROOT}/usr/lib/amdgcn/bitcode\" -target|g" \
		$(grep -l -r -e "-target" "${WORKDIR}") \
		|| die
	IFS=$' \t\n'
