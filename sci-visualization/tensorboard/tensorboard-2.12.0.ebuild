...
src_prepare() {
	...
	pushd "${WORKDIR}" || die
		eapply "${FILESDIR}/tensorboard-2.12.0-disable-versions.json-download.patch"
	popd
	sed -i -e "s|\.yarnrc|${WORKDIR}/.yarnrc|g" WORKSPACE || die
	sed -i -e "s|\.cache/yarn2|${HOME}/.cache/yarn2|g" WORKSPACE || die
	bazel_setup_bazelrc

	mkdir -p "${WORKDIR}/tensorboard-${PV}-bazel-base/external/go_sdk" || die
	# Snapshotted for determinism.
	cp -a \
		"${FILESDIR}/versions.json_20230330" \
		"${WORKDIR}/tensorboard-${PV}-bazel-base/external/go_sdk/versions.json" \
		|| die
}
