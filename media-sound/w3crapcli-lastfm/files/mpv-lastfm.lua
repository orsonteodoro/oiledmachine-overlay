...
function scrobble()
	...
	os.execute(string.format("lastfm.pl '%s' '%s' '%s' %d %d", esc(artist), esc(title), esc(album), length, 0))
end
...
function scrobbleNowPlaying()
	...
	if artist and title then
		...
		msg.info(string.format("Setting Now Playing %s - %s", artist, title))

		-- Using https://github.com/l29ah/w3crapcli/blob/master/last.fm/lastfm.pl
		os.execute(string.format("lastfm.pl '%s' '%s' '%s' %d %d", esc(artist), esc(title), esc(album), length, 1))

	end

	--schedule next call
	tim2 = mp.add_timeout(30, scrobbleNowPlaying) --every 30 seconds
end
...
function enqueue()
	...
	if artist and title then
		...
		if not length then
			length = 31	-- FIXME the old API sucks: it returns OK if the length is not specified/is 0/is -1, but doesn't scrobble anything.
		end
