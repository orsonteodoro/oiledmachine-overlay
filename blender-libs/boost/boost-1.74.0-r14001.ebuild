...
# Abbreviated prefix
prfx() {
	echo "/usr/$(get_libdir)/blender/boost/${CXXABI}"
}

# Abbreviated Prefixed ED
aed() {
	echo "${ED}$(prfx)"
}
...
src_configure() {
	...
	# Use C++${CXXABI} which is required by Blender
...
multilib_src_install_all() {
	if ! use numpy; then
		rm -r "$(aed)"/usr/include/boost/python/numpy* || die
	fi
	...
	if use python; then
		if use mpi; then
			...
			rm -r "$(aed)"/usr/include/boost/mpi/python* || die
		fi
	else
		rm -r "$(aed)"/usr/include/boost/{python*,mpi/python*,parameter/aux_/python,parameter/python*} || die
	fi

	if ! use nls; then
		rm -r "$(aed)"/usr/include/boost/locale || die
	fi

	if ! use context; then
		rm -r "$(aed)"/usr/include/boost/context || die
		rm -r "$(aed)"/usr/include/boost/coroutine{,2} || die
		rm "$(aed)"/usr/include/boost/asio/spawn.hpp || die
	fi
	...
	if use doc; then
		...
		dosym ../../../../include/boost $(prfx)/usr/share/doc/${PF}/html/boost
	fi
}
...
multilib_src_install() {
	ejam \
		"${OPTIONS[@]}" \
		--includedir="$(aed)/usr/include" \
		--libdir="$(aed)/usr/$(get_libdir)" \
		install || die "Installation of Boost libraries failed"

	pushd "$(aed)/usr/$(get_libdir)" >/dev/null || die

	local ext=$(get_libname)
	if use threads; then
		local f
		for f in *${ext}; do
			dosym ${f} $(prfx)/usr/$(get_libdir)/${f/${ext}/-mt${ext}}
		done
	fi
	...
	if [[ ${CHOST} == *-darwin* ]]; then
		...
		local d
		for d in "$(aed)"/usr/lib/*.dylib; do
			if [[ -f ${d} ]]; then
				# fix the "soname"
				ebegin "  correcting install_name of ${d#$(aed)}"
				install_name_tool -id "/${d#$(aed)}" "${d}"
				eend $?
...
pkg_preinst() {
	...
	local symlink
	for symlink in "${EROOT}"$(prfx)/usr/include/boost "${EROOT}"$(prfx)/usr/share/boostbook; do
		if [[ -L ${symlink} ]]; then
			rm -f "${symlink}" || die
		fi
	done
	...
	# Bug: 607734
	rm -rf "${EROOT}"$(prfx)/usr/include/boost-1_[3-5]? || die
}
...
pkg_postinst() {
	...
	elog "    <char>::transform_primary[abi:cxx${CXXABI}](char const*, char const*) const'"
	elog
