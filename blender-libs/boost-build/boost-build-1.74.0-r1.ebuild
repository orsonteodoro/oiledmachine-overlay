...
iprfx() {
        echo "/usr/$(get_libdir)/blender/${PN}/${MAJOR_V}/usr"
}
...
src_install() {
	exeinto $(iprfx)/bin
	doexe engine/{bjam,b2}
	fperms 0755 $(iprfx)/bin/{bjam,b2}
	fowners root:root $(iprfx)/bin/{bjam,b2}

	insinto $(iprfx)/share/boost-build
	doins -r "${FILESDIR}/site-config.jam" \
		../boost-build.jam bootstrap.jam build-system.jam ../example/user-config.jam *.py \
		build kernel options tools util

	find "${ED}"$(iprfx)/share/boost-build -iname '*.py' -delete || die
