...
inherit autotools check-reqs eutils flag-o-matic gnome2 multilib multilib-minimal pax-utils python-any-r1 subversion toolchain-funcs versionator virtualx
...
IUSE="aqua coverage debug +egl +geoloc gles2 +gstreamer +introspection +jit libsecret +opengl spell +webgl +X bmalloc ftl-jit"
# bugs 372493, 416331
REQUIRED_USE="
...
src_unpack() {
	unpack ${A}
	if use ftl-jit ; then
	        ESVN_REPO_URI="http://svn.webkit.org/repository/webkit/releases/WebKitGTK/webkit-2.4.9/Source/JavaScriptCore/llvm/"
	        ESVN_STORE_DIR="${T}"
	        ESVN_REVISION="192128"
	        subversion_src_unpack
		cp -r "${T}/webkit-gtk/llvm" "${WORKDIR}/${MY_P}/Source/JavaScriptCore"
	        ESVN_REPO_URI="http://svn.webkit.org/repository/webkit/releases/WebKitGTK/webkit-2.4.9/Source/JavaScriptCore/ftl/"
	        ESVN_STORE_DIR="${T}"
	        ESVN_REVISION="192128"
	        subversion_src_unpack
		cp "${T}/webkit-gtk/ftl/FTLStackMaps.h" "${WORKDIR}/${MY_P}/Source/JavaScriptCore/ftl"
		cp "${T}/webkit-gtk/ftl/FTLStackMaps.cpp" "${WORKDIR}/${MY_P}/Source/JavaScriptCore/ftl"
		cp "${T}/webkit-gtk/ftl/FTLInlineCacheSize.h" "${WORKDIR}/${MY_P}/Source/JavaScriptCore/ftl"
		cp "${T}/webkit-gtk/ftl/FTLInlineCacheSize.cpp" "${WORKDIR}/${MY_P}/Source/JavaScriptCore/ftl"
		cp "${T}/webkit-gtk/ftl/FTLSaveRestore.h" "${WORKDIR}/${MY_P}/Source/JavaScriptCore/ftl"
		cp "${T}/webkit-gtk/ftl/FTLSaveRestore.cpp" "${WORKDIR}/${MY_P}/Source/JavaScriptCore/ftl"
		cp "${T}/webkit-gtk/ftl/FTLSlowPathCallKey.h" "${WORKDIR}/${MY_P}/Source/JavaScriptCore/ftl"
		cp "${T}/webkit-gtk/ftl/FTLSlowPathCallKey.cpp" "${WORKDIR}/${MY_P}/Source/JavaScriptCore/ftl"
		cp "${T}/webkit-gtk/ftl/FTLSlowPathCall.h" "${WORKDIR}/${MY_P}/Source/JavaScriptCore/ftl"
		cp "${T}/webkit-gtk/ftl/FTLSlowPathCall.cpp" "${WORKDIR}/${MY_P}/Source/JavaScriptCore/ftl"
		cp "${T}/webkit-gtk/ftl/FTLLocation.h" "${WORKDIR}/${MY_P}/Source/JavaScriptCore/ftl"
		cp "${T}/webkit-gtk/ftl/FTLLocation.cpp" "${WORKDIR}/${MY_P}/Source/JavaScriptCore/ftl"
	        ESVN_REPO_URI="http://svn.webkit.org/repository/webkit/releases/WebKitGTK/webkit-2.4.9/Source/JavaScriptCore/disassembler/"
	        ESVN_STORE_DIR="${T}"
	        ESVN_REVISION="192128"
	        subversion_src_unpack
		cp -r "${T}/webkit-gtk/disassembler/udis86" "${WORKDIR}/${MY_P}/Source/JavaScriptCore/disassembler"
		cp "${T}/webkit-gtk/disassembler/UDis86Disassembler.h" "${WORKDIR}/${MY_P}/Source/JavaScriptCore/disassembler"
		cp "${T}/webkit-gtk/disassembler/UDis86Disassembler.cpp" "${WORKDIR}/${MY_P}/Source/JavaScriptCore/disassembler"
	fi
        #ESVN_REPO_URI="http://svn.webkit.org/repository/webkit/releases/WebKitGTK/webkit-2.4.9"
        #ESVN_STORE_DIR="${T}"
        #ESVN_REVISION="192128"
        #subversion_src_unpack
	#mv "${T}/webkit-gtk/webkit-${PV}" "${WORKDIR}/${MY_P}"
}
...
#		-i Tools/TestWebKitAPI/GNUmakefile.am || die
	...
	epatch "${FILESDIR}"/webkit-gtk-2.8.5-secretvalue.patch

	#epatch "${FILESDIR}"/webkit-gtk-2.4.9-llvmapi.h-1.patch
	#epatch "${FILESDIR}"/webkit-gtk-2.4.9-llvmapi.h-2.patch

	epatch "${FILESDIR}"/webkit-gtk-2.4.9-gnumakefilelistam.patch
	epatch "${FILESDIR}"/webkit-gtk-2.4.9-gnumakefileam.patch
...
multilib_src_configure() {
	...
	local ftljitcfg
	if [[ ${ABI} == "x86"  ]] ; then
		#mycmakeargs+=( --libexecdir=/usr/lib32/misc )
		mycmakeargs+=( --bindir=/usr/lib32/webkit-gtk )
		if use ftl-jit ; then
			_LLVM_LIBS="-L/usr/lib32 `llvm-config --libs`"
			ftljitcfg="--disable-ftl-jit"
		fi
	elif [[ ${ABI} == "amd64"  ]] ; then
		#mycmakeargs+=( --libexecdir=/usr/lib64/misc )
		mycmakeargs+=( --bindir=/usr/lib64/webkit-gtk )
		if use ftl-jit ; then
			_LLVM_LIBS="-L/usr/lib64 `llvm-config --libs`"
			ftljitcfg="--enable-ftl-jit"
		fi
	else
		#mycmakeargs+=( --libexecdir=/usr/$(get_libdir)/misc )
		mycmakeargs+=( --bindir=/usr/$(get_libdir)/webkit-gtk )
		if use ftl-jit ; then
			_LLVM_LIBS="-L/usr/$(get_libdir) `llvm-config --libs`"
			ftljitcfg="--enable-ftl-jit"
		fi
	fi

	if use bmalloc; then
		mycmakeargs+=( --enable-fast-malloc )
	else
		mycmakeargs+=( --disable-fast-malloc )
	fi

	mycmakeargs+=( --libdir=/usr/$(get_libdir) )
	LLVM_LIBS="${_LLVM_LIBS}" \
	ECONF_SOURCE=${S} \
...
multilib_src_install() {
	...
	if [[ ${ABI} == "x86"  ]] ; then
		use jit && pax-mark m "${ED}usr/lib32/webkit-gtk/jsc-1"
	elif [[ ${ABI} == "amd64"  ]] ; then
		use jit && pax-mark m "${ED}usr/lib64/webkit-gtk/jsc-1"
	else
