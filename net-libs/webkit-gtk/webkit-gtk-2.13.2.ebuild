...
inherit check-reqs cmake-utils eutils flag-o-matic gnome2 multilib multilib-minimal pax-utils python-any-r1 toolchain-funcs versionator virtualx
...
DEPEND="${RDEPEND}
	...
	test? (
		...
		jit? ( sys-apps/paxctl ) )
	ftl-jit? ( sys-devel/llvm
                ftl-native-call-inlining? ( sys-devel/clang ) )
"
...
multilib_src_configure() {
	...
	local mycmakeargs=(
		...
		$(cmake-utils_use_enable threaded-compositor THREADED_COMPOSITOR)
		$(cmake-utils_use_enable accelerated-overflow-scrolling ACCELERATED_OVERFLOW_SCROLLING)
		$(cmake-utils_use_enable accelerated-2d-canvas ACCELERATED_2D_CANVAS)
		$(cmake-utils_use_enable ftl-jit FTL_JIT)
		$(cmake-utils_use_enable ftl-native-call-inlining FTL_NATIVE_CALL_INLINING)
		-DCMAKE_BUILD_TYPE=Release
		-DPORT=GTK
		-DENABLE_PLUGIN_PROCESS_GTK2=ON
		-DENCHANT_INCLUDE_DIRS=/usr/include/enchant
		${ruby_interpreter}
	)
	if $(tc-getLD) --version | grep -q "GNU gold"; then
...
multilib_src_install() {
	...
	use jit && pax-mark m "${ED}usr/$(get_libdir)/webkit-gtk/jsc" "${ED}usr/$(get_libdir)/misc/webkit2gtk-4.0/WebKitWebProcess"
	pax-mark m "${ED}usr/$(get_libdir)/misc/webkit2gtk-4.0/WebKitPluginProcess"{,2}
}
