...
IUSE="cpu_flags_x86_sse2 debug doc +icu inspector man +npm pax_kernel +snapshot +ssl +system-ssl systemtap test"
REQUIRED_USE="
	inspector? ( icu ssl )
	npm? ( ssl )
	system-ssl? ( ssl )
...
RESTRICT="test"
S="${WORKDIR}/node-v${PV}"
NPM_V="6.14.6" # See https://github.com/nodejs/node/blob/v10.20.1/deps/npm/package.json
...
"Found higher slots, manually change the headers with \`eselect nodejs\`."
	...
	cp "${FILESDIR}/node-multiplexer-v1" "${EROOT}/usr/bin/node" || die
