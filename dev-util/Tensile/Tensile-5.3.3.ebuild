...
RESTRICT="test"
SLOT="${ROCM_SLOT}/${PV}"
IUSE="+client cuda +opencl +openmp +rocm ebuild-revision-17"
REQUIRED_USE="
	client? (
		${ROCM_REQUIRED_USE}
		openmp
...
RDEPEND="
	...
	rocm? (
		...
		openmp? (
			sys-libs/llvm-roc-libomp:${ROCM_SLOT}[${LLVM_ROC_LIBOMP_5_3_AMDGPU_USEDEP}]
		)
	)
"
DEPEND="
...
src_configure() {
	...
	if use client; then
		check_pkg_glibcxx "dev-libs/boost" "/usr/$(get_libdir)/libboost_program_options.so" "${HIP_5_3_GCC_SLOT}"
