...
RDEPEND="
	...
	rocm? (
		...
		openmp? (
			sys-libs/llvm-roc-libomp:${ROCM_SLOT}[${LLVM_ROC_LIBOMP_5_6_AMDGPU_USEDEP}]
		)
	)
"
DEPEND="
...
src_configure() {
	...
	if use client; then
		check_pkg_glibcxx "dev-libs/boost" "/usr/$(get_libdir)/libboost_program_options.so" "${HIP_5_6_GCC_SLOT}"
