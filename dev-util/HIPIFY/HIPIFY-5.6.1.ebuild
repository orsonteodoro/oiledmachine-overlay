...
pkg_setup() {
	if ! use test ; then
		:
	elif has_version "=dev-util/nvidia-cuda-toolkit-12.1*" && has_version "=llvm-core/clang-17*" && has_version "=llvm-core/llvm-17*" ; then
		LLVM_SLOT=17
	elif has_version "=dev-util/nvidia-cuda-toolkit-12.1*" && has_version "=llvm-core/clang-16*" && has_version "=llvm-core/llvm-16*" ; then
		LLVM_SLOT=16
