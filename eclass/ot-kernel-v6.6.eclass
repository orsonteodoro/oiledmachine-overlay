...
# Using -r1 in ot-sources-6.6.1-r1.ebuild is not allowed.
# Using _p20260704 in ot-sources-6.6.1_p20260704.ebuild is not allowed.
...
if [[ "${PV}" =~ "_rc" ]] ; then
	# ot-sources-6.6_rc1.ebuild
	RC_PV=$(ver_cut 4 "${PV}")
	MY_PV="${KV_MAJOR_MINOR}_${RC_PV}"		# 6.6_rc1
	UPSTREAM_PV="${KV_MAJOR_MINOR}-${RC_PV}"	# 6.6-rc1
else
	# ot-sources-6.6.9999.ebuild
	# ot-sources-6.6.ebuild
	# ot-sources-6.6.1.ebuild
	RC_PV=""
	MY_PV="${PV}" #					# 6.6, 6.6.1, 6.6.9999
	UPSTREAM_PV="${PV}"				# 6.6, 6.6.1, 6.6.9999
fi

# AMDGPU_FIRMWARE_RELEASE_DATE is based on firmware names from
# https://elixir.bootlin.com/linux/v6.6.143/source/drivers/gpu/drm/amd/display/include/dal_types.h	DCN 3.2.1
# https://elixir.bootlin.com/linux/v6.6.143/source/drivers/gpu/drm/amd/amdgpu/amdgpu_vcn.c		VCN 4.0.4
# https://elixir.bootlin.com/linux/v6.6.143/source/drivers/gpu/drm/amd/amdgpu/gfx_v11_0_3.c		the last gfx version for gc_11_0_4 and others with .bin reference
...
RTL_BT_FIRMWARE_RELEASE_DATE="20250106" # Based on rtl8723cs_xx_config bin referenced in https://github.com/torvalds/linux/blob/v6.6/drivers/bluetooth/btrtl.c
RTL8XXXU_FIRMWARE_RELEASE_DATE="20230517" # Based on latest added rtl8192fufw bin from https://github.com/torvalds/linux/blob/v6.6/drivers/net/wireless/realtek/rtl8xxxu/
RTLWIFI_FIRMWARE_RELEASE_DATE="20191008" # Based on latest added rtl8812aefw_wowlan bin from https://github.com/torvalds/linux/blob/v6.6/drivers/net/wireless/realtek/rtlwifi/
...
CLANG_PGO_SUPPORTED=0 # Needs updated patch
# See
...
tresor_prompt tresor_sysfs video_cards_nvidia zen-sauce
"
