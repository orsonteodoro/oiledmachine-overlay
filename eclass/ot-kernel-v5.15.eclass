...
# Using -r1 in ot-sources-5.15.1-r1.ebuild is not allowed.
# Using _p20260704 in ot-sources-5.15.1_p20260704.ebuild is not allowed.
...
if [[ "${PV}" =~ "_rc" ]] ; then
	# ot-sources-5.15_rc1.ebuild
	RC_PV=$(ver_cut 4 "${PV}")
	MY_PV="${KV_MAJOR_MINOR}_${RC_PV}"		# 5.15_rc1
	UPSTREAM_PV="${KV_MAJOR_MINOR}-${RC_PV}"	# 5.15-rc1
else
	# ot-sources-5.15.9999.ebuild
	# ot-sources-5.15.ebuild
	# ot-sources-5.15.1.ebuild
	RC_PV=""
	MY_PV="${PV}" #					# 5.15, 5.15.1, 5.15.9999
	UPSTREAM_PV="${PV}"				# 5.15, 5.15.1, 5.15.9999
fi
...
${PPC_FLAGS[@]}
${X86_FLAGS[@]}
amdgpu-dkms bbrv2 build c2tcp cfi +cfs -clang deepcc -debug doc -dwarf4 -dwarf5 -dwarf-auto
-exfat -expoline -gdb +genpatches -genpatches_1510 -lto nest multigen_lru orca
pgo prjc qt5 +retpoline rt shadowcallstack symlink tresor tresor_prompt
tresor_sysfs uksm video_cards_nvidia zen-multigen_lru zen-sauce
"
