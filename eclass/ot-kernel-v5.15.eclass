...
ewarn
	...
	if [[ -z "${OT_KERNEL_BUILDCONFIGS_5_15}" ]] ; then
		OT_KERNEL_BUILDCONFIGS_5_15_="ot:build:/etc/kernels/kernel-config-${K_MAJOR_MINOR}-ot-$(uname -m):$(uname -m):${CHOST}:cfs"
		if use rt ; then
			# Split for security reasons
			OT_KERNEL_BUILDCONFIGS_5_15_="${OT_KERNEL_BUILDCONFIGS_5_15_};rt:build:/etc/kernels/kernel-config-${K_MAJOR_MINOR}-rt-$(uname -m):$(uname -m):${CHOST}:cfs"
		fi
	else
