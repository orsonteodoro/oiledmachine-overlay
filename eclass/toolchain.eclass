...
# Added fix_param_max_lto_streaming_parallelism
# This ebuild fork is temporary until the 32 spawned lto1-wpa-streaming bug is fixed upstream.
...
fix_param_max_lto_streaming_parallelism() {
	# This should be implemented in GCC and checked instantenously and
	# dynamically adjusted over time but it is difficult to implement in C.

	# It been a problem since GCC 10 (circa 2020).

	# It's not really a core issue but more of a memory issue that can lead
	# to OOM with hard resets on poorly configured systems with less than 16 GiB of memory
	# or buggy patched kernels that break the OOM killer.
	local ram_mib=0
	local total_mib=0
	ram_mib=$(($(grep -r -e "MemTotal" /proc/meminfo  | grep -E -o -e "[0-9]+") / 1024))
	swap_mib=$(($(grep -r -e "SwapTotal" /proc/meminfo  | grep -E -o -e "[0-9]+") / 1024))

	# For this to work properly at its optimal performance, the number of
	# lto1-wpa-streaming spawned is based on ram_free_mib, considering swap
	# overhead and swap free space.

	local ram_free_mib=$(($(grep "MemFree" /proc/meminfo | grep -E -o -e "[0-9]+") / 1024))
	local swap_free_mib=$(($(grep "SwapFree" /proc/meminfo | grep -E -o -e "[0-9]+") / 1024 ))

	local min_lto1_streaming_size_mib=820 # 600 MiB swap and 220 MiB RSS
	local max_lto1_streaming_size_mib=1120 # 900 MiB swap and 220 MiB RSS

	# -1 process is for a safety cushion (aka variance) for anything like
	# increased memory use over versions.

	local total_mib=$(( ${ram_mib} + ${swap_mib} ))
	local free_mib=$(( ${ram_free_mib} + ${swap_free_mib} ))

	if (( ${free_mib} < ${max_lto1_streaming_size_mib} )) ; then
		eerror
		eerror "You need to add swap space to contain at least 1 lto1-wpa-streaming,"
		eerror "so at least ${max_lto1_streaming_size_mib} MiB is required."
		eerror
		die
	fi

	# Ideally we want 1 lto1-wpa-streaming per core, but you need again
	# ncores * ${max_lto1_streaming_size_mib} of free memory.

	# People forget it not just about the cores but also total memory
	# consumed per core on average is about a little less than 1 GiB
	# per core when lto-ing.

	# This check assumes one CPU die not SMP setup.  This is why
	# OVERRIDE_NCORES can be used to override if the check below is buggy.
	local ncores=1
	if [[ -n "${OVERRIDE_NCORES}" ]] ; then
		ncores=${OVERRIDE_NCORES}
	else
		ncores=$(lscpu | grep -e "Core(s) per socket" | grep -E -o -e "[0-9]+")
	fi

	# -1 for thrashless lto1-wpa-streaming.  The max_lto1_streaming_size_mib
	...
	if ! use vanilla ; then
		if (( ${ram_free_mib} < ${active_mem_mib} )) ; then
			...
			ewarn "combined used by ${ncores} cores which could result in trashing."
			ewarn
		fi

		if (( ${free_mib} < ${active_mem_mib} )) ; then
			eerror
			eerror "Free memory is not enough.  Free memory should be"
			eerror "${active_mem_mib} MiB or more.  Add more swapfile space."
			eerror
			die
		else
			einfo
			einfo "param_max_lto_streaming_parallelism memory scale report:"
			einfo
			einfo "RAM free MiB: ${ram_free_mib}"
			einfo "Total Free MiB: ${free_mib}"
			einfo "Active MiB: ${active_mem_mib} for ${ncores} cores consumed"
			einfo
		fi
		...
		local ncores=32
		local active_mem_mib=$(( ${ncores} * ${max_lto1_streaming_size_mib} ))
		# These checks assume the worst case.  You may actually need -4 GiB less than
		# reported here.
		if grep -q -e "Y" "/sys/module/zswap/parameters/enabled" ; then
			einfo "Checking compressed mem"
			# We cannot use linux-info in stage 2 yet
			# If using compressed pages, the compressed ratio is around 35/16 = 2.1875.
			local ratio=2.1875
			cat <<EOF > "${T}/memtest.py"
import sys;
import math;
active_mem_mib=math.ceil(${active_mem_mib}/${ratio})
print("Is ${free_mib} < " + str(active_mem_mib)  + "?")
if ${free_mib} < active_mem_mib:
	sys.exit(0)
else:
	sys.exit(1)
EOF
			if python "${T}/memtest.py" ; then
				active_mem_mib=$(python -c "import math;print(math.ceil(${active_mem_mib}/${ratio}))")
				eerror
				eerror "You need ${active_mem_mib} MiB of memory.  Add more swap."
				eerror
				die
			fi
		elif (( ${total_mib} < ${active_mem_mib} )) ; then
			# Only for uncompressed pages
			eerror
			eerror "You need ${active_mem_mib} MiB of total memory for lto1-wpa-streaming"
			eerror "add more swap or use zswap with a >= ${ratio} compression ratio algorithm."
			eerror
			die
		else
			einfo
			einfo "Sufficient memory for ${ncores} processes of lto1-wpa-streaming."
			einfo
		fi
	fi
}
