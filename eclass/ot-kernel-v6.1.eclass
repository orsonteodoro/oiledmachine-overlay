...
# Using -r1 in ot-sources-6.1.1-r1.ebuild is not allowed.
# Using _p20260704 in ot-sources-6.1.1_p20260704.ebuild is not allowed.
...
if [[ "${PV}" =~ "_rc" ]] ; then
	# ot-sources-6.1_rc1.ebuild
	RC_PV=$(ver_cut 4 "${PV}")
	MY_PV="${KV_MAJOR_MINOR}_${RC_PV}"		# 6.1_rc1
	UPSTREAM_PV="${KV_MAJOR_MINOR}-${RC_PV}"	# 6.1-rc1
else
	# ot-sources-6.1.9999.ebuild
	# ot-sources-6.1.ebuild
	# ot-sources-6.1.1.ebuild
	RC_PV=""
	MY_PV="${PV}" #					# 6.1, 6.1.1, 6.1.9999
	UPSTREAM_PV="${PV}"				# 6.1, 6.1.1, 6.1.9999
fi
...
RTLWIFI_FIRMWARE_RELEASE_DATE="20191008" # Based on latest added rtl8812aefw_wowlan bin from https://github.com/torvalds/linux/blob/v6.1/drivers/net/wireless/realtek/rtlwifi/
RTW_FIRMWARE_RELEASE_DATE="20221014" # Based on latest added rtw8852b_fw bin driver from https://github.com/torvalds/linux/blob/v6.1/drivers/net/wireless/realtek/rtw89
