...
# @ECLASS: blender-v2.81.eclass
# @MAINTAINER: orsonteodoro@hotmail.com
# @BLURB: blender implementation
# @DESCRIPTION:
# The blender-v2.81.eclass helps reduce code duplication across ebuilds
...
IUSE+=" X +abi5-compat abi6-compat abi7-compat +alembic -asan +boost +bullet \
-collada -color-management -cpudetection +cuda +cycles -cycles-network +dds \
-debug doc +elbeem -embree -ffmpeg -fftw flac -jack +jemalloc +jpeg2k -llvm \
-man +ndof +nls +nvcc -nvrtc +openal +opencl -openexr -openimagedenoise \
-openimageio +openmp -opensubdiv -openvdb -optix -osl release -sdl -sndfile \
+tbb test +tiff -valgrind"
...
REQUIRED_USE+="
	...
	openvdb? ( ^^ ( abi5-compat abi6-compat abi7-compat ) boost openexr tbb )
...
# versions.cmake last date inspected: Oct 28, 2019
...
RDEPEND+=" ${PYTHON_DEPS}
	...
	virtual/opengl
	media-libs/libglvnd
	alembic? ( >=media-gfx/alembic-1.7.8[boost(+),hdf(+)] )
	boost? ( ${BOOST_DEPEND} )
	collada? (
