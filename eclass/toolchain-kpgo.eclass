...
# oteodoro:  Added MY_PN, modified ${PN} -> ${MY_PN}
MY_PN="gcc"
MY_FLAVOR="-kpgo"
...
if tc_is_live ; then
	...
	EGIT_BRANCH="releases/${MY_PN}-${PV%.?.?_pre9999}"
...
PREFIX=${TOOLCHAIN_PREFIX:-${EPREFIX}/usr/lib/gcc-kpgo}

# oteodoro:  Kernel PGO changes to avoid merge conflict
...
if [[ ${MY_PN} != kgcc64 && ${MY_PN} != gcc-* ]] ; then
...
gcc_do_make() {
	...
	einfo "Compiling ${MY_PN} (${GCC_MAKE_TARGET})..."
...
toolchain-kpgo_src_test() {
	...
	# oteodoro:  renamed /gcc/ -> /gcc${MY_FLAVOR}/
	if [[ ! -d "${BROOT}"/var/cache/gcc${MY_FLAVOR}/${SLOT} ]] && ! [[ ${success_tests} -eq 0 ]] ; then
		# We have no reference data saved from a previous run to know if
		# the failures are tolerable or not, so we bail out.
		eerror "Reference test data does NOT exist at ${BROOT}/var/cache/gcc${MY_FLAVOR}/${SLOT}"
	...
	if [[ -d "${BROOT}"/var/cache/gcc${MY_FLAVOR}/${SLOT} ]] ; then
		einfo "Comparing with previous cached results at ${BROOT}/var/cache/gcc-pgo/${SLOT}"

		# Exit with the following values:
		# 0 if there is nothing of interest
		# 1 if there are errors when comparing single test case files
		# N for the number of errors found when comparing directories
		"${S}"/contrib/compare_tests "${BROOT}"/var/cache/gcc${MY_FLAVOR}/${SLOT}/ . || die "Comparison for tests results failed, error code: $?"
	fi
}

#---->> src_install <<----

toolchain-kpgo_src_install() {
	cd "${WORKDIR}"/build || die
...
# oteodoro:  renamed /gcc/ -> /gcc${MY_FLAVOR}/
	#  - "${D}${PREFIX}/libexec/gcc-pkgo/${CTARGET}/${GCC_CONFIG_VER}"
	# Using ${CTARGET} tools:
	#  - "${D}${LIBPATH}"
...
# oteodoro:  renamed /gcc/ -> /gcc${MY_FLAVOR}/
	pax-mark -r "${ED}/libexec/gcc${MY_FLAVOR}/${CTARGET}/${GCC_CONFIG_VER}/cc1"
	pax-mark -r "${ED}/libexec/gcc${MY_FLAVOR}/${CTARGET}/${GCC_CONFIG_VER}/cc1plus"
...
# oteodoro:  renamed /gcc/ -> /gcc${MY_FLAVOR}/
		einfo "Copying test results to ${EPREFIX}/var/cache/gcc${MY_FLAVOR}/${SLOT} for future comparison"
		(
			dodir /var/cache/gcc${MY_FLAVOR}/${SLOT}
			cd "${WORKDIR}"/build || die
			find . -name \*.sum -exec cp --parents -v {} "${ED}"/var/cache/gcc${MY_FLAVOR}/${SLOT} \;
		)
	fi
}
...
create_gcc_env_entry() {
	dodir /etc/env.d/gcc${MY_FLAVOR} # oteodoro:  changed gcc -> gcc${MY_FLAVOR}

	local gcc_envd_base="/etc/env.d/gcc${MY_FLAVOR}/${CTARGET}-${GCC_CONFIG_VER}" # oteodoro:  changed gcc -> gcc${MY_FLAVOR}
...
toolchain-kpgo_pkg_postinst() {
	...
	if ! is_crosscompile && [[ ${MY_PN} != "kgcc64" ]] ; then
...
toolchain-kpgo_pkg_postrm() {
	...
	if is_crosscompile ; then
		# oteodoro:  renamed /gcc/ -> /gcc${MY_FLAVOR}/
		if [[ -z $(ls "${EROOT}"/etc/env.d/gcc${MY_FLAVOR}/${CTARGET}* 2>/dev/null) ]] ; then
...
do_gcc_config() {
	...
	if [[ -n ${current_gcc_config} ]] ; then
		...
		if [[ -n ${use_specs} ]] && \
		   # oteodoro:  renamed /gcc/ -> /gcc${MY_FLAVOR}/
		   [[ ! -e ${EROOT}/etc/env.d/gcc${MY_FLAVOR}/${CTARGET}-${GCC_CONFIG_VER}${use_specs} ]]
		then
