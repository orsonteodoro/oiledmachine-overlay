...
# Using -r1 in ot-sources-5.10.1-r1.ebuild is not allowed.
# Using _p20260704 in ot-sources-5.10.1_p20260704.ebuild is not allowed.
...
if [[ "${PV}" =~ "_rc" ]] ; then
	# ot-sources-5.10_rc1.ebuild
	RC_PV=$(ver_cut 4 "${PV}")
	MY_PV="${KV_MAJOR_MINOR}_${RC_PV}"		# 5.10_rc1
	UPSTREAM_PV="${KV_MAJOR_MINOR}-${RC_PV}"	# 5.10-rc1
else
	# ot-sources-5.10.9999.ebuild
	# ot-sources-5.10.ebuild
	# ot-sources-5.10.1.ebuild
	RC_PV=""
	MY_PV="${PV}" #					# 5.10, 5.10.1, 5.10.9999
	UPSTREAM_PV="${PV}"				# 5.10, 5.10.1, 5.10.9999
fi
