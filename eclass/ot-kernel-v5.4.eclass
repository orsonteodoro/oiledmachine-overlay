...
ewarn
	...
	if [[ -z "${OT_KERNEL_BUILDCONFIGS_5_4}" ]] ; then
		OT_KERNEL_BUILDCONFIGS_5_4_="ot:build:/etc/kernels/kernel-config-${K_MAJOR_MINOR}-ot-$(uname -m):$(uname -m):${CHOST}:cfs"
		if use rt ; then
			# Split for security reasons
			OT_KERNEL_BUILDCONFIGS_5_4_="${OT_KERNEL_BUILDCONFIGS_5_4_};rt:build:/etc/kernels/kernel-config-${K_MAJOR_MINOR}-rt-$(uname -m):$(uname -m):${CHOST}:cfs"
		fi
	else
