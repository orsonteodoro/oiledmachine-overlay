...
einfo "Patched vulnerabilities:"
		local severity_label=${SEVERITY_LABEL:-"CVSS 3.1"} # Can be CVSS 4.0 or a custom severity label
		IFS=$'\n'
		local x
...
echo -e " \033[0;32m*\033[0m \e[9m${id}\e[0m${_delimiter}  ${vulnerability_classes}${_severity}"
			else
...
einfo "IV = Insufficient Validation of Input or Data"
		...
		if [[ "${VULNERABILITIES_FIXED[@]}" =~ (^|" "|";"|",")"JITM"($|" "|";"|",") ]] ; then
...
# See also BGC, JITM
einfo "PGC = Poorly Generated Code"
		fi
...
einfo "SOF = Shift Overflow"
		...
		if [[ "${VULNERABILITIES_FIXED[@]}" =~ (^|" "|";"|",")"STCKCRRPTN"($|" "|";"|",") ]] ; then
einfo "STCKCRRPTN = Stack Corruption"
		fi
