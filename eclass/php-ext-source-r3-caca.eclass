...
# @ECLASS: php-ext-source-r3-caca.eclass
# @MAINTAINER:
...
[[ -z "${PHP_EXT_NAME}" ]] && \
	die "no extension name specified for the php-ext-source-r3-caca eclass"
...
# @CODE
[[ -z "${USE_PHP}" ]] && \
	die "USE_PHP is not set for the php-ext-source-r3-caca eclass"
...
# @DEFAULT_UNSET
# @DESCRIPTION:
# By default, we run "phpize" in php-ext-source-r3-caca_src_prepare(). Set
...
# @FUNCTION: php-ext-source-r3-caca_src_prepare
# @DESCRIPTION:
...
# and calls php-ext-source-r3-caca_phpize().
...
# @FUNCTION: php-ext-source-r3-caca_phpize
# @DESCRIPTION:
...
# @FUNCTION: php-ext-source-r3-caca_src_configure
# @DESCRIPTION:
...
# @FUNCTION: php-ext-source-r3-caca_src_compile
# @DESCRIPTION:
...
# @FUNCTION: php-ext-source-r3-caca_src_install
# @DESCRIPTION:
...
php_init_slot_env() {
	...
	PHP_EXT_S="${WORKDIR}/${slot}/caca-php" # changed by oiledmachine-overlay
...
# @FUNCTION: php-ext-source-r3-caca_createinifiles
# @DESCRIPTION:
	...
	for slot in $(php_get_slots); do
		...
		for file in $(php_slot_ini_files "${slot}") ; do
			if [[ "${PHP_EXT_INI}" = "yes" ]] ; then
				...
				php-ext-source-r3-caca_addextension "${PHP_EXT_NAME}.so" "${file}"
			fi
...
# @FUNCTION: php-ext-source-r3-caca_addextension
...
# php-ext-source-r3-caca_addtoinifile to do the actual work.
php-ext-source-r3-caca_addextension() {
	...
	php-ext-source-r3-caca_addtoinifile "${2}" "${ext_type}" "${ext_file}"
}

# @FUNCTION: php-ext-source-r3-caca_addtoinifile
...
php-ext-source-r3-caca_addtoinifile() {
...
# @FUNCTION: php-ext-source-r3-caca_addtoinifiles
...
#
# php-ext-source-r3-caca_addtoinifiles "zend_optimizer.optimization_level" "15"
# php-ext-source-r3-caca_addtoinifiles "zend_optimizer.enable_loader" "0"
# php-ext-source-r3-caca_addtoinifiles "zend_optimizer.disable_licensing" "0"
#
# Adding values to a section in php.ini file installed by the extension:
#
# php-ext-source-r3-caca_addtoinifiles "[Debugger]"
# php-ext-source-r3-caca_addtoinifiles "debugger.enabled" "on"
# php-ext-source-r3-caca_addtoinifiles "debugger.profiler_enabled" "on"
# @CODE
php-ext-source-r3-caca_addtoinifiles() {
	local slot
	for slot in $(php_get_slots); do
		for file in $(php_slot_ini_files "${slot}") ; do
			php-ext-source-r3-caca_addtoinifile "${file}" "${1}" "${2}"
		done
	done
}
