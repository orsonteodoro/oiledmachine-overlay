...
# @OUTPUT_VARIABLE
# @DESCRIPTION:
# The version components without the -r, _p, _pre components used in *DEPENDs contexts.
if [[ "${PV}" =~ "9999" ]] ; then
	LLVM_VERSION=$(ver_cut 1-4)
else
	LLVM_VERSION=$(ver_cut 1-3)
fi

# @ECLASS_VARIABLE: LLVM_VERSION_CANONICAL
# @OUTPUT_VARIABLE
# @DESCRIPTION:
# The 3 version components only in upstream or code contexts.
LLVM_VERSION_CANONICAL=$(ver_cut 1-3)
