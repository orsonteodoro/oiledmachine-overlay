...
FFMPEG_IUSE+="
	+jpeg2k libaom +mp3 +opus rav1e svt-av1 +theora +vorbis +vpx webm +webp x264 +xvid
"
...
gen_oidn_depends() {
	...
	for s in ${LLVM_COMPAT[@]} ; do
		echo "
		llvm_slot_${s}? (
			>=media-libs/oidn-1.4.3[llvm_slot_${s},aot?,sycl?]
			<media-libs/oidn-1.5[llvm_slot_${s},aot?,sycl?]
		)
		"
	done
}
	...
	!patent_status_nonfree? (
		ffmpeg? (
			|| (
				...
				media-video/ffmpeg:0/58.60.60[encode,jpeg2k?,libaom?,mp3?,opus?,rav1e?,sdl,svt-av1?,theora?,vorbis?,vpx?,-x264,xvid?,zlib]
			)
		)
	)
	patent_status_nonfree? (
...
RDEPEND+="
	...
	openvdb? (
		abi10-compat? (
			|| (
				=media-gfx/openvdb-12*[${PYTHON_SINGLE_USEDEP},abi10-compat,blosc,numpy]
				=media-gfx/openvdb-11*[${PYTHON_SINGLE_USEDEP},abi10-compat,blosc,numpy]
				=media-gfx/openvdb-10*[${PYTHON_SINGLE_USEDEP},abi10-compat,blosc,numpy]
			)
		)
