...
if [[ "${PV}" != "9999" ]] ; then
	BDEPEND+="
		verify-sig? (
			>=sec-keys/openpgp-keys-lassecollin-20250313
		)
	"
fi
...
einfo "Testing '${test_variant}' variant"
				"${BUILD_DIR}/src/xz/xz" -c ${test_variant} "xz-pgo-test-01.tar" \
					| "${BUILD_DIR}/src/xz/xz" -c -d - \
					> /dev/null
		...
		if tc-is-clang; then
			llvm-profdata \
				merge \
				"${T}/${ABI}-pgo" \
				--output="${T}/${ABI}-pgo/default.profdata" \
				|| die
		fi
