...
# Build/install only progress for 2.18.0:
# CPU - pass
...
# See https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/xla/xla/stream_executor/device_description.h#L235
	gfx900
	gfx906
	gfx908
	gfx90a
...
CUDA_TARGETS_COMPAT=(
# See https://github.com/tensorflow/tensorflow/blob/v2.18.0/.bazelrc#L246  # Supported upstream
# See https://github.com/tensorflow/tensorflow/blob/v2.18.0/.bazelrc#L670  # Unsupported upstream
	sm_60 # Supported
	sm_70 # Supported
	sm_80 # Supported
	sm_89 # Supported
...
inherit bazel check-reqs cuda distutils-r1 dhms flag-o-matic lcnr llvm multibuild
...
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/.bazelversion
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/configure.py#L33							# cuda version
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/lite/tools/cmake/modules/eigen.cmake
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/tools/ci_build/Dockerfile.rbe.rocm-ubuntu18.04-manylinux2010-multipython#L19 # rocm version min
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/tools/ci_build/Dockerfile.rbe.rocm-ubuntu20.04-manylinux2014-multipython#L20 # rocm version max
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/tools/dockerfiles/partials/ubuntu/nvidia.partial.Dockerfile	# cuda/cudnn major.minor versions
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/tools/pip_package/setup.py#L176				# cuda version
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/tools/ci_build/release/requirements_common.txt		# python deps versions ; pinned
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/tools/ci_build/release/requirements_ubuntu.txt		# python deps versions ; pinned ; depends on requirements_common.txt
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/tools/pip_package/setup.py#L84				# python deps versions
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/tools/tf_sig_build_dockerfiles/devel.requirements.txt	# python deps versions ; pinned ; depends on requirements_common.txt
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/tools/toolchains/archives.bzl
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/tools/toolchains/remote_config/containers.bzl		# containers for testing
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/tools/toolchains/remote_config/configs.bzl#L318		# tested llvm
...
# zlib:
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl#L567
# google-cloud-cpp:
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl#L303

# https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl#L542			# openmp
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/absl/workspace.bzl			# abseil-cpp ; provides commit
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/flatbuffers/workspace.bzl		# See also https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/lite/schema/schema_generated.h
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/gemmlowp/workspace.bzl
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/gpus/rocm_configure.bzl#L191        # llvms supported for rocm
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/hwloc/workspace.bzl
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/icu/workspace.bzl
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/jpeg/workspace.bzl
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/llvm/workspace.bzl
...
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/llvm_openmp/openmp.bzl
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/pasta/workspace.bzl
# https://github.com/grpc/grpc/blob/b54a5b338637f92bfcf4b0bc05e0f57a5fd8fadd/CMakeLists.txt
# https://github.com/tensorflow/tensorflow/blob/v2.18.0/ci/official/requirements_updater/requirements.in
...
ABSEIL_PY_PV="1.0.0"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
APPLE_SUPPORT_PV="1.6.0"	# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
CUDA_PV="12.5"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/tools/pip_package/setup.py#L176
BAZEL_SKYLIB_PV="1.3.0"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace3.bzl
CUB_PV="1.9.9"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
CUDNN_FRONTEND_PV="1.6.1"	# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
FLATBUFFERS_PV="24.3.25"	# From https://github.com/google/flatbuffers/blob/e6463926479bd6b330cbcf673f7e917803fd5831/CMake/Version.cmake
GRPC_PV="1.53.0"		# Based on the oldest grpc supporting abseil 20230125
GRPCIO_PV="1.24.3"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/tools/pip_package/setup.py#L84
GRPCIO_PV_MAX="1.53"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/tools/pip_package/setup.py#L84 ; < (Exclusive) ; Upstream is wrong
KISSFFT_PV="131.1.0"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/kissfft/workspace.bzl
NCCL_PV="2.19.3-1"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
ONEDNN_PV="3.5"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
OOURA_FFT_PV="1.0"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
OPENMP_PV="10.0.1"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
#PLATFORMS_PV="0.0.6"		# From https://github.com/tensorflow/runtime/blob/4e088077c6e455610a39ed8d2a18fe195ada6137/third_party/rules_cuda/cuda/dependencies.bzl#L66 with EGIT_COMMIT_TF_RUNTIME
PROTOBUF_PV="3.21.9"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
...
RULES_ANDROID_PV="0.1.1"	# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
RULES_APPLE_PV="2.3.0"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
...
RULES_JVM_PV="4.3"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace3.bzl
RULES_PKG_PV="0.7.1"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace3.bzl
RULES_PYTHON_PV="0.1.0"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/WORKSPACE#L19
RULES_SWIFT_PV="1.5.0"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
SNAPPY_PV="1.2.0"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl#L594
TRITON_TAG="cl675928942"	# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/triton/workspace.bzl#L8
ZSTD_PV="1.4.5"
# RULES_DOCKER dumped?

EGIT_COMMIT_ABSEIL_CPP="fb3621f4f897824c0dbe0615fa94543df6192f30"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/absl/workspace.bzl
EGIT_COMMIT_ARM_NEON_2_X86_SSE="a15b489e1222b2087007546b4912e21293ea86ff"	# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
EGIT_COMMIT_BENCHMARK="f7547e29ccaed7b64ef4f7495ecfff1c9f6f3d03"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/benchmark/workspace.bzl
EGIT_COMMIT_BAZEL_TOOLCHAINS="8c717f8258cd5f6c7a45b97d974292755852b658"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace1.bzl
EGIT_COMMIT_BROTLI="3914999fcc1fda92e750ef9190aa6db9bf7bdb07"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl#L888
EGIT_COMMIT_CPUINFO="fa1c679da8d19e1d87f20175ae1ec10995cd3dd3"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
EGIT_COMMIT_DLPACK="2a7e9f1256ddc48186c86dff7a00e189b47e5310"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/dlpack/workspace.bzl
EGIT_COMMIT_DUCC="aa46a4c21e440b3d416c16eca3c96df19c74f316"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/ducc/workspace.bzl#L6
EGIT_COMMIT_FARMHASH="0d859a811870d10f53a594927d0d0b97573ad06d"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/farmhash/workspace.bzl
EGIT_COMMIT_FLATBUFFERS="e6463926479bd6b330cbcf673f7e917803fd5831"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/flatbuffers/workspace.bzl
EGIT_COMMIT_FP16="4dfe081cf6bcd15db339cf2680b9281b8451eeb3"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/FP16/workspace.bzl
EGIT_COMMIT_FXDIV="63058eff77e11aa15bf531df5dd34395ec3017c8"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
EGIT_COMMIT_GEMMLOWP="16e8662c34917be0065110bfcd9cc27d30f52fdf"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/gemmlowp/workspace.bzl
EGIT_COMMIT_GOOGLEAPIS="6b3fdcea8bc5398be4e7e9930c693f0ea09316a0"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl#L305
EGIT_COMMIT_HIGHWAYHASH="c13d28517a4db259d738ea4886b1f00352a3cc33"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/highwayhash/workspace.bzl
EGIT_COMMIT_IMPLIB_SO="2cce6cab8ff2c15f9da858ea0b68646a8d62aef2"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/implib_so/workspace.bzl#L9
EGIT_COMMIT_LIBEIGEN="33d0937c6bdf5ec999939fb17f2a553183d14a74"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/eigen3/workspace.bzl
EGIT_COMMIT_LLVM="8b4b7d28f7c344c728a9812aa99d9ad24edb40a2"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/llvm/workspace.bzl
EGIT_COMMIT_ML_DTYPES="6f02f77c4fa624d8b467c36d1d959a9b49b07900"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/py/ml_dtypes/workspace.bzl#L10
EGIT_COMMIT_PTHREADPOOL="4fe0e1e183925bf8cfa6aae24237e724a96479b8"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
EGIT_COMMIT_PYBIND11_ABSEIL="2c4932ed6f6204f1656e245838f4f5eae69d2e29"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/pybind11_abseil/workspace.bzl
EGIT_COMMIT_PYBIND11_BAZEL="72cbbf1fbc830e487e3012862b7b720001b70672"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/pybind11_bazel/workspace.bzl
EGIT_COMMIT_PYBIND11_PROTOBUF="80f3440cd8fee124e077e2e47a8a17b78b451363"	# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl#L790
EGIT_COMMIT_RE2="03da4fc0857c285e3a26782f6bc8931c4c950df4"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl ; Round up to the first day of cur_month + 1
EGIT_COMMIT_RIEGELI="5d75119232cd4f6db8dfa69a1503289f050e9643"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl#L872
EGIT_COMMIT_RULES_CLOSURE="308b05b2419edb5c8ee0471b67a40403df940149"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace3.bzl
EGIT_COMMIT_RULES_PROTO="11bf7c25e666dd7ddacbcd4d4c4a9de7a25175f8"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace0.bzl
EGIT_COMMIT_RUY="3286a34cc8de6149ac6844107dfdffac91531e72"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/ruy/workspace.bzl
EGIT_COMMIT_SOBOL_DATA="835a7d7b1ee3bc83e575e302a985c66ec4b65249"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/sobol_data/workspace.bzl
EGIT_COMMIT_STABLEHLO="9bb28f84c281795783639364b727e4398dcec570"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/stablehlo/workspace.bzl
EGIT_COMMIT_TF_RUNTIME="4e088077c6e455610a39ed8d2a18fe195ada6137"		# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/tf_runtime/workspace.bzl
EGIT_COMMIT_XNNPACK="6b83f69d4938da4dc9ad63c00bd13e9695659a51"			# From https://github.com/tensorflow/tensorflow/blob/v2.18.0/tensorflow/workspace2.bzl
...
https://github.com/google/snappy/archive/${SNAPPY_PV}.tar.gz -> snappy-${SNAPPY_PV}.tar.gz
	...
	python? (
		$(python_gen_cond_dep '
			|| (
				...
				=sci-ml/tensorflow-io-0.37.1[${PYTHON_SINGLE_USEDEP},tensorflow-io-gcs-filesystem]
			)
		' python3_{10,11})
...
PATCHES=(
	...
	"${FILESDIR}/2.18.0/tensorflow-2.18.0-0008-Relax-setup.py-version-requirements.patch"
	"${FILESDIR}/2.18.0/tensorflow-2.15.0-0009-systemlib-update-targets-for-absl.patch"
	"${FILESDIR}/2.18.0/tensorflow-2.15.0-0010-systemlib-fix-missing-osx-in-pybind11.patch"
	"${FILESDIR}/2.18.0/tensorflow-2.15.0-0011-systemlib-fix-missing-LICENSE-in-flatbuffers.patch"
	"${FILESDIR}/2.18.0/tensorflow-2.18.0-0013-build-use-non-hermetic-python.patch"
)
ROCM_PATCHES=(
...
einfo
	...
	# See https://github.com/tensorflow/tensorflow/blob/v2.18.0/third_party/systemlibs/syslibs_configure.bzl
		local SYSLIBS=(
