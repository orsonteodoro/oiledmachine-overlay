...
CUTLASS_COMMIT_1="44c704eae85da352d277d6f092f41412772f70e4"
...
DYNOLOG_COMMIT="950cd3f80a3b1d966314380e8ecdd47ee0c586cc" # kineto dep ; committer-date:<=2023-10-31
FBGEMM_COMMIT="88fc6e741bc03e09fcdc3cd365fa3aafddb7ec24"
FFMPEG_COMPAT=(
...
FMT_COMMIT_1="f5e54359df4c26b6230fc61d38aa294581393084"
FMT_COMMIT_2="d9063baf227882da0f48c761abcbb08247eb1296" # kineto dep ; committer-date:<=2023-10-31
...
GLOO_COMMIT="cf1e1abc95d0b961222ee82b6935f76250fbcf16"
GOOGLETEST_COMMIT_1="d66ce585109eed6d2d891b7ed7ab3ca96e854483" # gloo dep ; committer-date:<=2023-07-18
GOOGLETEST_COMMIT_2="5ec7f0c4a113e2f18ac2c6cc7df51ad6afc24081" # protobuf dep
GOOGLETEST_COMMIT_3="518387203b573f35477fa6872dd54620e70d2bdb" # kineto dep ; committer-date:<=2023-10-31
...
ONNX_COMMIT_1="ccde5da81388ffa770ca98b64e07f803ad089414"
...
RDEPEND="
	...
	system-libs? (
		...
		>=sci-ml/onnx-1.15.0_rc2
		cuda? (
			>=dev-libs/cudnn-frontend-0.9.2:0/8
		)
		fbgemm? (
			>=sci-ml/FBGEMM-2023.12.01
		)
		gloo? (
		...
		if ! use clang ; then
			if has_version "=dev-util/nvidia-cuda-toolkit-12.1*" ; then
				...
				if use cpu_flags_riscv_rvv ; then
					...
					min_slot=11
				elif use cpu_flags_arm_bf16 ; then
					min_slot=10
...
src_prepare() {
	...
	if use system-libs ; then
		eapply "${FILESDIR}/${PN}-2.2.2-cuda-hardcoded-paths.patch"
		eapply "${FILESDIR}/${PN}-2.2.2-rocm-hardcoded-paths.patch"
	else
		eapply "${FILESDIR}/${PN}-2.2.2-cuda-hardcoded-paths.patch"
		eapply "${FILESDIR}/${PN}-2.2.2-rocm-hardcoded-paths.patch"
		eapply "${FILESDIR}/${PN}-2.2.2-cuda-hardcoded-paths-third-party.patch"
		eapply "${FILESDIR}/${PN}-2.2.2-rocm-hardcoded-paths-third-party.patch"
	fi
