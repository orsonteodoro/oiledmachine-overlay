...
_PATCHES=(
	...
	"${FILESDIR}/caffe2-2.2.2-cuda-hardcoded-paths.patch"
	"${FILESDIR}/caffe2-2.2.2-rocm-hardcoded-paths.patch"
)
...
src_prepare() {
	...
	if [[ -n "${ROCM_VERSION}" ]] ; then
		...
		sed -i -e "s|@ROCM_VERSION@|${HIP_5_7_VERSION}|g" $(grep -r -l "@ROCM_VERSION@" "${WORKDIR}") || die
	fi
