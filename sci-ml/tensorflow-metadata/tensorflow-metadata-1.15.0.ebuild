...
BAZEL_SKYLIB_PV="1.0.2"							# https://github.com/tensorflow/metadata/blob/v1.15.0/WORKSPACE#L9
BAZEL_SLOT="6.1"							# https://github.com/tensorflow/metadata/blob/v1.15.0/WORKSPACE#L52
DISTUTILS_USE_PEP517="setuptools"
PROTOBUF_PV="3.21.9"							# https://github.com/tensorflow/metadata/blob/v1.15.0/WORKSPACE#L14
...
ZLIB_PV="1.2.12"							# https://github.com/tensorflow/metadata/blob/v1.15.0/WORKSPACE#L31
...
if [[ "${PV}" =~ "9999" ]] ; then
	...
	FALLBACK_COMMIT="47aaaa7aa820fd8a865babcfddbdafe11ec92a25" # Apr 22, 2023
	inherit git-r3
else
	KEYWORDS="~amd64"
bazel_external_uris="
...
DEPEND+="
	${RDEPEND}
"
# Upstream wants protobuf 4 but tensorflow ebuilds uses 0/3.21 for Python 3.11
BDEPEND+="
	(
...
src_unpack() {
	if [[ "${PV}" =~ "9999" ]] ; then
		...
		git-r3_fetch
		git-r3_checkout
		grep -q -e "__version__ = '1.15.0'" "${S}/tensorflow_metadata/version.py" \
			|| die "QA:  Bump version"
	else
		unpack ${A}
	fi
