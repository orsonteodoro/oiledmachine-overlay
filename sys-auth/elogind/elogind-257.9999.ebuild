...
CFLAGS_HARDENED_USE_CASES="multiuser-system security-critical untrusted-data sensitive-data"

PYTHON_COMPAT=( python3_{10..14} )

if [[ ${PV} = *9999* ]]; then
	FALLBACK_COMMIT="d7956ed0b1496db7ff404d1765c31cbd9f6c7aab"
	EGIT_BRANCH="main"
...
inherit cflags-hardened chkl eapi9-ver linux-info meson pam python-any-r1 udev secure-version xdg-utils
...
src_unpack() {
	...
	local actual_slot=$(grep -r -e "LIBSYSTEMD_.*{" "${S}/src/libelogind/libelogind.sym" | tail -n 1 | grep -E -o "[0-9]+")
	local expected_slot="${PV%%.*}"
	if ver_test "${actual_slot}" "-ne" "${expected_slot}" ; then
eerror "QA:  Bump the version:"
