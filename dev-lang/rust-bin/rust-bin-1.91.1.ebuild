...
if [[ ${PV} == *9999* ]]; then
	...
	SRC_URI="$(rust_all_arch_uris "rust-${PV}")
		rust-src? ( ${RUST_TOOLCHAIN_BASEURL%/}/2025-11-10/rust-src-${PV}.tar.xz )
		ppc64? ( elibc_musl? ( !big-endian? (
			$(rust_arch_uri powerpc64le-unknown-linux-musl rust-${PV})
		) ) )
	"
	#KEYWORDS="amd64 arm arm64 ~loong ~mips ppc ppc64 ~riscv ~s390 ~sparc x86"
	KEYWORDS="amd64 arm arm64 ~loong ppc ~s390 x86"
fi
...
if [[ ${PV} != *9999* && ${PV} != *beta* ]]; then
	...
	[[ "${KEYWORDS}" =~ "sparc" ]] && SRC_URI+=" sparc? ( ${GENTOO_BIN_BASEURI}/rust-${PVR}-sparc64-unknown-linux-gnu.tar.xz ) "
	[[ "${KEYWORDS}" =~ "mips" ]] && SRC_URI+=" mips? (
		abi_mips_o32? (
