...
IUSE+="
${IUSE_VAAPI}
+alsa aja amf +browser +browser-panels browser-qt-loop -decklink -fdk ftl
imagemagick +ipv6 jack +lua +new-mpegts-output nvafx nvenc nvvfx oss +pipewire
+pulseaudio +python +rtmps +speexdsp -test +freetype +hevc mac-syphon qsv11
+rnnoise -service-updates -sndio +speexdsp +v4l2 vaapi vlc +virtualcam +vst
+wayland win-dshow +websocket +whatsnew
...
#	fi
	...
	local mycmakeargs=(
		...
		-DENABLE_OSS=$(usex kernel_FreeBSD ON $(usex oss ON OFF))
