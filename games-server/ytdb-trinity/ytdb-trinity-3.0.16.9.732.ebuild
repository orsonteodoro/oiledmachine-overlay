...
src_unpack() {
	...
	ESVN_REPO_URI="http://svn2.assembla.com/svn/ytdbase/Trinity/Cataclysm/R73"
	ESVN_STORE_DIR="${WORKDIR}"
	ESVN_REVISION="449"
	subversion_src_unpack
...
src_install() {
	mkdir -p "${D}/usr/share/trinity/${SLOT}"
	cp -R "${WORKDIR}"/* "${D}/usr/share/trinity/${SLOT}"
	cp "${FILESDIR}/install-ytdb-trinity.sh" "${D}/usr/share/trinity/${SLOT}"
