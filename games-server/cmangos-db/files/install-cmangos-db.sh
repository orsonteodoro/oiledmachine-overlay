...
get_realmd_version() {
	local FOUND="0"
	mysql -N -B --user="$USER" --password="$PASSWORD" ${PREFIX}_${ID_AUTH} -e "SHOW COLUMNS FROM realmd_db_version" | grep -P "required_[rcz]?[0-9]+_[0-9]+"
	if [[ "$?" == "0" ]] ; then
		#required_20000_11_Creature_Template_repair scheme
	...
	mysql --user="$USER" --password="$PASSWORD" ${PREFIX}_${ID_AUTH} -e "SHOW COLUMNS FROM realmd_db_version" | grep -P "required_"
	if [[ "${FOUND}" == "0" && "$?" == "0" ]] ; then
		REALMD_VERSION=$(mysql --user="$USER" --password="$PASSWORD" ${PREFIX}_${ID_AUTH} -e "SELECT * FROM realmd_db_version" | grep "required" | sed -r -e "s|.*required_[rcz]?([0-9_]+).*|\1|g" -e "s|[_]?$||g" -e "s|_|.|g")
		FOUND="1"
	fi
