...
get_realmd_version() {
	...
	if [[ "$?" == "0" ]] ; then
		#required_20000_11_Creature_Template_repair scheme
		MANGOS_VERSION=$(mysql -N -B --user="$USER" --password="$PASSWORD" ${PREFIX}_${ID_AUTH} -e "SHOW COLUMNS FROM realmd_db_version" | grep "required_" | sed -r -e "s|required_[rcz]?([0-9]+)_([0-9]+).*|\1.\2|g")
		FOUND="1"
	fi
...
if [[ "$MODE" =~ "new" ]]; then
	TF=`tempfile`
	if [[ "$ENGINE" =~ "trinitycore" ]]; then
		f="sql/create/create_mysql.sql"
		cat "$f" | sed -e "s|characters|${PREFIX}_${ID_CHARACTERS}|g" -e "s|${ID_AUTH}|${PREFIX}_${ID_AUTH}|g" -e "s|\`${ID_WORLD}\`|\`${PREFIX}_${ID_WORLD}\`|g" > $TF
	else
	...
	check_success
	if [[ "$ENGINE" =~ "trinitycore" ]]; then
		add_db "sql/base/${ID_CHARACTERS}_database.sql" "${ID_CHARACTERS}"
		add_db "sql/base/${ID_AUTH}_database.sql" "${ID_AUTH}"
	else
...
if [[ "$MODE" =~ "unsafe_update" || "$MODE" =~ "new" ]]; then
