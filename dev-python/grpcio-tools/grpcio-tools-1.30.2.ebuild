...
RDEPEND="
	>=dev-cpp/abseil-cpp-20200225.0:${ABSEIL_CPP_SLOT}[${LIBCXX_USEDEP},${LIBSTDCXX_USEDEP},cxx_standard_cxx11?,cxx_standard_cxx14?,cxx_standard_cxx17?]
	dev-cpp/abseil-cpp:=
	...
	dev-python/protobuf:=
	~dev-python/grpcio-${PV}:${GRPC_SLOT}[${LIBCXX_USEDEP},${LIBSTDCXX_USEDEP},${PYTHON_USEDEP},cxx_standard_cxx11?,cxx_standard_cxx14?,cxx_standard_cxx17?]
	dev-python/grpcio:=
"
DEPEND="
	${RDEPEND}
...
python_configure() {
	...
	local L2=(
		$(PKG_CONFIG_PATH="\
${ESYSROOT}/usr/lib/abseil-cpp/${ABSEIL_CPP_SLOT}/${libdir}/pkgconfig:\
${ESYSROOT}/usr/lib/protobuf/${PROTOBUF_CPP_SLOT}/${libdir}/pkgconfig:\
${ESYSROOT}/usr/lib/grpc/${GRPC_SLOT}/${libdir}/pkgconfig:\
...
src_install() {
	...
	# Change of base /usr -> /usr/lib/grpc/${GRPC_SLOT}
		...
		pv="${pv/.}"
		patchelf \
			--add-rpath "${ESYSROOT}/usr/lib/abseil-cpp/${ABSEIL_CPP_SLOT}/$(get_libdir)" \
			--add-rpath "${ESYSROOT}/usr/lib/protobuf/${PROTOBUF_CPP_SLOT}/$(get_libdir)" \
			$(realpath "${ED}/usr/lib/grpc/${GRPC_SLOT}/lib/${EPYTHON}/site-packages/grpc_tools/_protoc_compiler.cpython-${pv}-"*"-linux-gnu.so") \
			|| die
	}
	...
	local new_prefix="/usr/lib/grpc/${GRPC_SLOT}/share"
	mv "${ED}${old_prefix}" "${ED}${new_prefix}" || die
}
