...
RDEPEND="
	>=dev-cpp/abseil-cpp-20250512.1:${ABSEIL_CPP_SLOT}[${LIBCXX_USEDEP},${LIBSTDCXX_USEDEP}]
	dev-cpp/abseil-cpp:=
	dev-libs/protobuf:${PROTOBUF_CPP_SLOT}[${LIBCXX_USEDEP},${LIBSTDCXX_USEDEP}]
	dev-libs/protobuf:=
	>=dev-python/cython-3.1:${CYTHON_SLOT}[${PYTHON_USEDEP}]
	dev-python/cython:=
...
src_install() {
	...
	# Change of base /usr -> /usr/lib/grpc/${GRPC_SLOT}
		...
		local new_prefix="/usr/lib/grpc/${GRPC_SLOT}/lib/python-exec/${EPYTHON}"
		dodir "/usr/lib/grpc/${GRPC_SLOT}/lib/python-exec"
		mv "${ED}${old_prefix}" "${ED}${new_prefix}" || die
	...
	local new_prefix="/usr/lib/grpc/${GRPC_SLOT}/bin"
	mv "${ED}${old_prefix}" "${ED}${new_prefix}" || die
	...
	pushd "${ED}/usr/lib/grpc/${GRPC_SLOT}/bin" >/dev/null 2>&1 || die
...
# Test with:
# LD_LIBRARY_PATH="/usr/lib/protobuf/6/lib64:${LD_LIBRARY_PATH}" PYTHONPATH="/usr/lib/grpc/6/lib/python3.11/site-packages:${PYTHONPATH}" PATH="/usr/lib/grpc/6/bin:${PATH}" python-grpc-tools-protoc --help
