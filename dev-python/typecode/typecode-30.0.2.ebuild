...
PYTHON_COMPAT=( "python3_11" ) # List up to 3.10
...
if [[ "${PV}" =~ "9999" ]] ; then
	...
	EGIT_REPO_URI="https://github.com/aboutcode-org/typecode.git"
	FALLBACK_COMMIT="f0c628085f61b966b3ed5c03fd3ed5e0bd528a9f" # Aug 6, 2024
	...
	S="${WORKDIR}/${PN}-${PV}"
	SRC_URI="
https://github.com/aboutcode-org/typecode/archive/refs/tags/v${PV}.tar.gz
	-> ${P}.tar.gz
	"
fi
...
HOMEPAGE="
	...
	https://pypi.org/project/typecode
"
LICENSE="
	all-rights-reserved
	Apache-2.0
...
RDEPEND+="
	...
	>=dev-python/container-inspector-31.0.0[${PYTHON_USEDEP}]
	...
	>=dev-python/intbitset-3.0.2[${PYTHON_USEDEP}]
	...
	>=dev-python/plugincode-30.0.0[${PYTHON_USEDEP}]
	...
	>=dev-python/regipy-2.3.1[${PYTHON_USEDEP}]
	...
	>=dev-python/commoncode-30.2.0[${PYTHON_USEDEP}]
	>=app-text/pdfminer-20200101[${PYTHON_USEDEP}]
	>=dev-python/plugincode-30.0.0[${PYTHON_USEDEP}]
...
BDEPEND+="
	...
	dev? (
		>=dev-python/aboutcode-toolkit-8.0.0[${PYTHON_USEDEP}]
	...
	full? (
		>=dev-python/typecode-libmagic-5.39.210223[${PYTHON_USEDEP}]
	)
	test? (
		...
		dev-python/vendy[${PYTHON_USEDEP}]
		dev-python/isort[${PYTHON_USEDEP}]
	)
"
DOCS=( "AUTHORS.rst" "CHANGELOG.rst" "README.rst" "typecode.ABOUT" )
...
src_unpack() {
	if [[ "${PV}" =~ "9999" ]] ; then
		...
		git-r3_fetch
		git-r3_checkout
		grep -q -e "version = 30.0.1" "${S}/setup.cfg" \
			|| die "QA:  Bump version"
	else
		unpack ${A}
	fi
