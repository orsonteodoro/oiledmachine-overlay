...
if [[ "${PV}" =~ "9999" ]] ; then
	...
	FALLBACK_COMMIT="b51402436b71989f14218f70a955c65dd45aee7f" # Jul 18, 2024
	IUSE+=" fallback-commit"
	S="${WORKDIR}/${P}"
	inherit git-r3
else
	EGIT_COMMIT="b51402436b71989f14218f70a955c65dd45aee7f" # Jul 18, 2024
...
#
# Triton upstream uses 12.4, but the LLVM project accepts 11.8, 12.3.
#
# For CUDA compatibility, see
#
#   https://github.com/llvm/llvm-project/blob/llvmorg-19-init/clang/include/clang/Basic/Cuda.h#L44
#
# CUDA SDK ebuilds:
#
#   12.4:  https://gitweb.gentoo.org/repo/gentoo.git/tree/dev-util/nvidia-cuda-toolkit/nvidia-cuda-toolkit-12.4.1.ebuild?id=d071cb72002d9422a4d1d94160012d222196173c
#
RDEPEND+="
	!rocm? (
		$(gen_llvm_rdepend)
...
DOCS=( "README.md" )
_PATCHES=(
	"${FILESDIR}/${PN}-3.0.0_p1-dynlib.patch"
...
einfo "Called python_configure"
	...
	elif use llvm_slot_19 && has_version "llvm-core/llvm:19" && has_version "sys-devel/mlir:19" ; then
