...
nettle openmp openssl system-gocode system-godef system-gnulib system-racerd unicode"
...
RDEPEND="${PYTHON_DEPS}
	>=app-shells/bash-4
	dev-libs/nxjson
	dev-util/bear[${PYTHON_USEDEP}]
	dev-util/compdb[${PYTHON_USEDEP}]
	dev-util/ninja
...
src_configure() {
	...
	if use system-gocode ; then
		...
		gocode_path="$(python_get_sitedir)/third_party/gocode/gocode"
	fi
	if use system-godef ; then
		godef_path="/usr/bin/godef"
	else
		godef_path="$(python_get_sitedir)/third_party/godef/godef"
	fi
	if use system-racerd ; then
		racerd_path="/usr/bin/racerd"
	else
		racerd_path="$(python_get_sitedir)/third_party/racerd/racerd"
	fi
	...
	YCMD_PATH="$(python_get_sitedir)/ycmd/${YCMD_SLOT}/ycmd" \
