...
multilib_src_configure() {
	gnome2_src_configure \
		--program-suffix="-${ABI}" \
		$(use_with gtk) \
		$(multilib_native_usex introspection --enable-introspection --disable-introspection) \
		$(multilib_native_usex vala --enable-vala --disable-vala) \
...
multilib_src_install() {
	...
	if multilib_is_native_abi; then
		dosym /usr/bin/gcr-viewer-${ABI} \
			/usr/bin/gcr-viewer
		dosym /usr/libexec/gcr-ssh-askpass-${ABI} \
			/usr/libexec/gcr-ssh-askpass
		dosym /usr/libexec/gcr-prompter-${ABI} \
