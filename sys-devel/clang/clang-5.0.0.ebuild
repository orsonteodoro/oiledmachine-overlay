...
src_prepare() {
	...
	cd - >/dev/null || die
        ...
        if use codegen; then
                eapply "${FILESDIR}/${PN}-5.0.0-export-fpic-clang-driver.patch"
        fi
...
multilib_src_install() {
	...
	if use codegen; then
		...
		#required in the 9999 series
                cp VarBypassDetector.h "${D}"/usr/include/clang/CodeGen/

                mkdir -p "${D}/usr/include/clang/Driver"
                cd "${S}"/include/clang/Driver/
               	cp Tool.h "${D}"/usr/include/clang/Driver/ || die
                cp ToolChain.h "${D}"/usr/include/clang/Driver/ || die
        fi
}
