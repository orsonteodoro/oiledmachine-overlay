...
IUSE_3D="
+angle +csg +glslang +gltf +gridmap +lightmapper_rd +meshoptimizer +mobile-vr
...
+astc +bc -betsy +bmp +brotli +cvtt +dds +etc +exr +fbx +hdr +jpeg +ktx +minizip -mp1
-mp2 +mp3 +ogg +s3tc +svg +tga +theora +vorbis +webp
"
IUSE_GUI="
+dbus -editor-splash +visual-shader +wayland +X
"
IUSE_INPUT="
...
src_compile() {
	...
	options_modules+=(
		angle=$(usex angle)
		betsy_export_templates=$(usex betsy)
		brotli=$(usex brotli)
		...
		cvtt_export_templates=$(usex cvtt)
		debug_symbols=$(usex debug)
		dev_build=$(usex debug)
		graphite=$(usex graphite)
		mp3_extra_formats=$(usex mp2 True $(usex mp1 True False))
		...
		module_jolt_physics_enabled=$(usex jolt)
		...
		module_visual_shader_enabled=$(usex visual-shader)
...
# OILEDMACHINE-OVERLAY-TEST:  PASSED (4.7.2, 20260903)
