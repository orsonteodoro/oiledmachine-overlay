...
CARGO_MIN_VER="1.77.2"
RUST_MAX_VER="1.86.0" # inclusive, LTS zone
RUST_MIN_VER="1.77.2" # inclusive, LTS zone
RUST_MULTILIB=1
...
einfo "cargo version:"
	${CARGO} --version || die
	local cargo_pv=$("${CARGO}" --version | cut -f 2 -d " ")
	local rustc_pv=$("${RUSTC}" --version | cut -f 2 -d " ")
	if ver_test "${cargo_pv}" "-lt" "${CARGO_MIN_VER}" ; then
eerror
eerror "Cargo doesn't meet requirements"
eerror
eerror "Expected version:  >= ${CARGO_MIN_VER}"
eerror "Actual version:  ${cargo_pv}"
eerror
		die
	fi
