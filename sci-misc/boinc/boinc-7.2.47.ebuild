...
src_prepare() {
	...
	if use fix-idle ; then
		eapply "${FILESDIR}"/${PN}-7.2.44-r2-idlefix.patch
	fi
	eapply "${FILESDIR}"/${PN}-7.2.44-r2-glut-missing.patch

	sed -i -e "s|AM_LIBTOOLFLAGS =|AM_LIBTOOLFLAGS = --tag=CXX|g" Makefile.incl
...
pkg_preinst() {
	...
	#fix permissions
	chown -R boinc:boinc /var/lib/${PN}
}
