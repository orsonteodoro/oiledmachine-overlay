...
_PATCHES=(
	...
	"${FILESDIR}/caffe2-2.3.1-cuda-hardcoded-paths.patch"
	"${FILESDIR}/caffe2-2.3.1-rocm-hardcoded-paths.patch"
)
...
src_prepare() {
	...
	if [[ -n "${ROCM_VERSION}" ]] ; then
		...
		sed -i -e "s|@ROCM_VERSION@|${HIP_6_0_VERSION}|g" $(grep -r -l "@ROCM_VERSION@" "${WORKDIR}") || die
	fi
