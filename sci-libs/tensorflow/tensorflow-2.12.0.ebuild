...
LLVM_MAX_SLOT=16
LLVM_MIN_SLOT=15
LLVM_SLOTS=( ${LLVM_MAX_SLOT} ${LLVM_MIN_SLOT} ) # See https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/tools/toolchains/remote_config/configs.bzl
...
# https://github.com/google/boringssl/blob/b9232f9e27e5668bc0414879dcdedb2a59ea75f2/src/include/openssl/crypto.h#L99
# https://github.com/tensorflow/runtime/blob/91d765cad5599f9710973d3e34d4dc22583e2e79/third_party/rules_cuda/cuda/dependencies.bzl#L41	# cc_rules
# https://github.com/tensorflow/runtime/blob/91d765cad5599f9710973d3e34d4dc22583e2e79/third_party/rules_cuda/cuda/dependencies.bzl#L66	# platforms
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/.bazelversion
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/configure.py#L33                                                        # cuda version
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/lite/tools/cmake/modules/eigen.cmake
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/tools/dockerfiles/partials/ubuntu/nvidia.partial.Dockerfile	# cuda/cudnn major.minor versions
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/tools/ci_build/Dockerfile.rbe.rocm-ubuntu18.04-manylinux2010-multipython#L19 # rocm version min
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/tools/ci_build/Dockerfile.rbe.rocm-ubuntu20.04-manylinux2014-multipython#L20 # rocm version max
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/tools/ci_build/release/requirements_common.txt		# python deps versions ; pinned
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/tools/ci_build/release/requirements_ubuntu.txt		# python deps versions ; pinned ; depends on requirements_common.txt
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/tools/pip_package/setup.py#L84				# python deps versions
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/tools/tf_sig_build_dockerfiles/devel.requirements.txt	# python deps versions ; pinned ; depends on requirements_common.txt
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/tools/toolchains/archives.bzl
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/tools/toolchains/remote_config/containers.bzl		# containers for testing
...
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace2.bzl#L542			# openmp
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/absl/workspace.bzl			# abseil-cpp ; provides commit
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/flatbuffers/workspace.bzl
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/gemmlowp/workspace.bzl
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/gpus/rocm_configure.bzl#L191        # llvms supported for rocm
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/hwloc/workspace.bzl
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/icu/workspace.bzl
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/jpeg/workspace.bzl
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/llvm/workspace.bzl
#   https://github.com/llvm/llvm-project/blob/10939d1d580b9d3c9c2f3539c6bdb39f408179c0/llvm/CMakeLists.txt#L14  # same as llvm 17.0.0git
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/llvm_openmp/openmp.bzl
# https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/pasta/workspace.bzl
...
ABSEIL_PY_PV="1.0.0"		# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace2.bzl
APPLE_SUPPORT_PV="1.1.0"	# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace2.bzl
CUDA_PV="11.8"			# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/tools/pip_package/setup.py#L365
BAZEL_SKYLIB_PV="1.3.0"		# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace3.bzl
CUB_PV="1.9.9"			# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace2.bzl
...
OOURA_FFT_PV="1.0"		# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace2.bzl
...
RULES_PKG_PV="0.7.1"		# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace3.bzl
RULES_PYTHON_PV="0.0.1"		# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace2.bzl
RULES_SWIFT_PV="1.0.0"		# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace2.bzl
# RULES_DOCKER dumped?

EGIT_COMMIT_ABSEIL_CPP="273292d1cfc0a94a65082ee350509af1d113344d"		# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/absl/workspace.bzl
EGIT_COMMIT_ARM_NEON_2_X86_SSE="a15b489e1222b2087007546b4912e21293ea86ff"	# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace2.bzl
EGIT_COMMIT_BAZEL_TOOLCHAINS="8c717f8258cd5f6c7a45b97d974292755852b658"		# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace1.bzl
EGIT_COMMIT_CPUINFO="3dc310302210c1891ffcfb12ae67b11a3ad3a150"			# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace2.bzl
EGIT_COMMIT_DLPACK="9351cf542ab478499294864ff3acfdab5c8c5f3d"			# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/dlpack/workspace.bzl
EGIT_COMMIT_FARMHASH="0d859a811870d10f53a594927d0d0b97573ad06d"			# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/farmhash/workspace.bzl
EGIT_COMMIT_FP16="4dfe081cf6bcd15db339cf2680b9281b8451eeb3"			# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/FP16/workspace.bzl
EGIT_COMMIT_FXDIV="63058eff77e11aa15bf531df5dd34395ec3017c8"			# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace2.bzl
EGIT_COMMIT_GEMMLOWP="e844ffd17118c1e17d94e1ba4354c075a4577b88"			# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/gemmlowp/workspace.bzl
EGIT_COMMIT_HIGHWAYHASH="c13d28517a4db259d738ea4886b1f00352a3cc33"		# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/highwayhash/workspace.bzl
EGIT_COMMIT_LIBEIGEN="3460f3558e7b469efb8a225894e21929c8c77629"			# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/eigen3/workspace.bzl
EGIT_COMMIT_LLVM="10939d1d580b9d3c9c2f3539c6bdb39f408179c0"			# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/llvm/workspace.bzl
EGIT_COMMIT_PTHREADPOOL="b8374f80e42010941bda6c85b0e3f1a1bd77a1e0"		# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace2.bzl
EGIT_COMMIT_RE2="a276a8c738735a0fe45a6ee590fe2df69bcf4502"			# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace2.bzl ; Round up to the first day of cur_month + 1
EGIT_COMMIT_RULES_CC="081771d4a0e9d7d3aa0eed2ef389fa4700dfb23e"			# From https://github.com/tensorflow/runtime/blob/91d765cad5599f9710973d3e34d4dc22583e2e79/third_party/rules_cuda/cuda/dependencies.bzl#L41
EGIT_COMMIT_RULES_CLOSURE="308b05b2419edb5c8ee0471b67a40403df940149"		# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace3.bzl
EGIT_COMMIT_RULES_JAVA="7cf3cefd652008d0a64a419c34c13bdca6c8f178"		# By console inspection
EGIT_COMMIT_RULES_PROTO="11bf7c25e666dd7ddacbcd4d4c4a9de7a25175f8"		# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace0.bzl
EGIT_COMMIT_RUY="3286a34cc8de6149ac6844107dfdffac91531e72"			# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/ruy/workspace.bzl
EGIT_COMMIT_SNAPPY="984b191f0fefdeb17050b42a90b7625999c13b8d"			# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/tensorflow/workspace2.bzl#L567
EGIT_COMMIT_SOBOL_DATA="835a7d7b1ee3bc83e575e302a985c66ec4b65249"		# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/sobol_data/workspace.bzl
EGIT_COMMIT_STABLEHLO="51f005f0a8ff6e28f535adfec4de936cb4097aa4"		# From https://github.com/tensorflow/tensorflow/blob/v2.12.0/third_party/stablehlo/workspace.bzl
