...
"
SLOT="${ROCM_SLOT}/${PV}"
IUSE="comgr debug hiprtc kernels mlir opencl +rocm test ebuild_revision_11"
...
RDEPEND="
	...
	kernels? (
		~sci-libs/miopenkernels-${PV}:${ROCM_SLOT}[${MIOPENKERNELS_5_3_AMDGPU_USEDEP}]
	)
	opencl? (
		~dev-libs/rocm-opencl-runtime-${PV}:${ROCM_SLOT}[${LLVM_ROC_LIBOMP_5_3_AMDGPU_USEDEP}]
		~sci-libs/miopengemm-${PV}:${ROCM_SLOT}
	)
	rocm? (
		~dev-util/hip-${PV}:${ROCM_SLOT}[rocm]
		~sci-libs/rocBLAS-${PV}:${ROCM_SLOT}[${ROCBLAS_5_3_AMDGPU_USEDEP},rocm]
	)
"
DEPEND="
	${RDEPEND}
...
PATCHES=(
	...
	"${FILESDIR}/${PN}-5.1.3-mlir_build-vector-header.patch"
)
...
src_install() {
	...
	# Prevent collision
	rm -rf "${ED}/opt/rocm-5.3.3/share/miopen/db/"*".kbd" || true
}
