...
RDEPEND="
	...
	kernels? (
		~sci-libs/miopenkernels-${PV}:${ROCM_SLOT}[${MIOPENKERNELS_5_2_AMDGPU_USEDEP}]
	)
	opencl? (
		~dev-libs/rocm-opencl-runtime-${PV}:${ROCM_SLOT}[${LLVM_ROC_LIBOMP_5_2_AMDGPU_USEDEP}]
		~sci-libs/miopengemm-${PV}:${ROCM_SLOT}
	)
	rocm? (
		~dev-util/hip-${PV}:${ROCM_SLOT}[rocm]
		~sci-libs/rocBLAS-${PV}:${ROCM_SLOT}[${ROCBLAS_5_2_AMDGPU_USEDEP},rocm]
	)
"
DEPEND="
	${RDEPEND}
