...
# https://github.com/ROCm/MIOpen/blob/rocm-5.5.1/test/CMakeLists.txt#L100
	gfx803
	gfx900
	gfx906
	gfx908
...
"
SLOT="${ROCM_SLOT}/${PV}"
IUSE="comgr composable-kernel debug hiprtc kernels mlir opencl +rocm test ebuild-revision-7"
...
RDEPEND="
	...
	composable-kernel? (
		sci-libs/composable_kernel:${ROCM_SLOT}[${COMPOSABLE_KERNEL_5_5_AMDGPU_USEDEP}]
	)
	kernels? (
		~sci-libs/miopenkernels-${PV}:${ROCM_SLOT}[${MIOPENKERNELS_5_5_AMDGPU_USEDEP}]
	)
	opencl? (
		~dev-libs/rocm-opencl-runtime-${PV}:${ROCM_SLOT}[${LLVM_ROC_LIBOMP_5_5_AMDGPU_USEDEP}]
		=sci-libs/miopengemm-5.5*:0/5.5
	)
	rocm? (
		~dev-util/hip-${PV}:${ROCM_SLOT}[rocm]
		~sci-libs/rocBLAS-${PV}:${ROCM_SLOT}[${ROCBLAS_5_5_AMDGPU_USEDEP},rocm]
	)
"
DEPEND="
	${RDEPEND}
