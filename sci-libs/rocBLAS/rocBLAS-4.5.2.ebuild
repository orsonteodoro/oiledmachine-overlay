...
RDEPEND="
	...
	benchmark? (
		sys-libs/llvm-roc-libomp:${ROCM_SLOT}[${LLVM_ROC_LIBOMP_4_5_AMDGPU_USEDEP}]
		virtual/blas
	)
	cuda? (
		${HIP_CUDA_DEPEND}
	...
	rocm? (
		~dev-util/Tensile-${PV}:${ROCM_SLOT}[${TENSILE_4_5_AMDGPU_USEDEP}]
	)
"
DEPEND="
	${RDEPEND}
...
BDEPEND="
	...
	rocm? (
		$(python_gen_cond_dep '
			~dev-util/Tensile-'"${PV}:${ROCM_SLOT}"'['"${TENSILE_4_5_AMDGPU_USEDEP}"',${PYTHON_USEDEP},client]
		')
	)
	test? (
		dev-cpp/gtest
...
PATCHES=(
	...
	"${FILESDIR}/${PN}-4.5.2-cpp_lib_filesystem.patch"
