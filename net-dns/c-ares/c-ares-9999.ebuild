...
src_unpack() {
	...
	local c=$(grep -e "CARES_VERSION_INFO" "${S}/configure.ac" | head -n 1 | cut -f 2 -d "=" | sed -e 's|"||g' | cut -f 1 -d ":")
	local a=$(grep -e "CARES_VERSION_INFO" "${S}/configure.ac" | head -n 1 | cut -f 2 -d "=" | sed -e 's|"||g' | cut -f 3 -d ":")
	local sover_actual=$(( ${c} - ${a} ))
	local sover_expected="${SOVER}"
	if ver_test "${sover_actual}" "-ne" "${sover_expected}" ; then
eerror "QA:  Update the subslot to ${sover_actual}"
eerror "QA:  Actual sover:  ${sover_actual}"
eerror "QA:  Expected sover:  ${sover_expected}"
		die
	fi
}
