...
| `:<SLOT>` needs slot operator := in *DEPEND                     | Y                                                                             |
| C++ header only packages needs slot operator := in DEPEND       | Y                                                                             |
...
| Proactive REQUIRED_USE vs reactive configure/compile time build errors | REQUIRED_USE must be used for long compile times or long unpack times to resolve issues ahead of time |
...
| Proactive fatal error vs reactive fatal error                   | Producing a proactive fatal error in the ebuild is preferred because compile time or unpack cost can be very expensive.  The user resolves it ahead of time, but lazy programmers or developers that prioritize minimal ebuild size are okay with reactive. |
