...
| Low level vulnerabilities monitored                           | Double free, deadlock, heap overflow, integer overflow, memory leak, null pointer dereference, ROP gadgets, speculative execution, stack overflow, string format, uninitialized memory, use-after-free, type confusion |
| High level vulnerabilities monitored                          | Container/sandbox escapes, inappropriate configuration, inappropriate file permissions, JIT miscompilation, prototype pollution, SQL injection, TOCTOU, unseen/zero-day, zero-click |
