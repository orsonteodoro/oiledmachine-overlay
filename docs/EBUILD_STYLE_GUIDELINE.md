...
| Low level vulnerabilities monitored                           | Double free, deadlock, heap overflow, integer overflow, memory leak, miscompilation, null pointer dereference, ROP gadgets, speculative execution, stack overflow, string format, uninitialized memory, use after free, use after return, type confusion |
| High level vulnerabilities monitored                          | Container/sandbox escapes, inappropriate configuration, inappropriate file permissions, prototype pollution, injection, path traversal, TOCTOU, unsanitized input, unseen/zero-day, zero-click |
