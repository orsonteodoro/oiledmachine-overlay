...
SRC_URI="
	https://gitlab.gnome.org/GNOME/${PN}/-/archive/${PV}/${P}.tar.bz2
"
...
src_install() {
	...
	if use systemd ; then
		rm "${ED}/usr/lib/systemd/user/vte-spawn-.scope.d/defaults.conf" \
			|| die
	fi
	if use gtk-doc ; then
		mkdir -p "${ED}/usr/share/gtk-doc/" || die
		mv \
			"${ED}/usr/share/doc/vte-${SLOT}" \
			"${ED}/usr/share/gtk-doc/vte-${SLOT}-gtk3" \
			|| die
	fi
}
